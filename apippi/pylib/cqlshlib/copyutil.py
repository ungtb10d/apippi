...
from apippi import OperationTimedOut
from apippi.cluster import Cluster, DefaultConnection
from apippi.cqltypes import ReversedType, UserType, VarcharType
from apippi.metadata import protect_name, protect_names, protect_value
from apippi.policies import RetryPolicy, WhiteListRoundRobinPolicy, DCAwareRoundRobinPolicy, FallthroughRetryPolicy
from apippi.query import BatchStatement, BatchType, SimpleStatement, tuple_factory
from apippi.util import Date, Time
...
class FilesReader(object):
    ...
    def __init__(self, fname, options):
        ...
        self.fname = fname
        self.sources = None  # might be initialised directly here? (see apippi-17350)
        self.num_sources = 0
...
class FeedingProcess(mp.Process):
    ...
    def __init__(self, inpipe, outpipe, worker_pipes, fname, options):
        ...
        self.inmsg = None  # might be initialised directly here? (see apippi-17350)
        self.outmsg = None  # might be initialised directly here? (see apippi-17350)
        self.worker_channels = None  # might be initialised directly here? (see apippi-17350)
...
class ChildProcess(mp.Process):
    ...
    def on_fork(self):
        """
        Create the channels and release any parent connections after forking, see apippi-11749 for details.
        """
        self.inmsg = ReceivingChannel(self.inpipe)
        self.outmsg = SendingChannel(self.outpipe)
...
class ImportConversion(object):
    ...
    def _get_converter(self, cql_type):
        ...
        into BoundStatement.bind() for the given cql type. See apippi.cqltypes
        for more details.
        """
        ...
        def convert_user_type(val, ct=cql_type):
            ...
            wrong order, so we must sort them according to ct.fieldnames, see apippi-12959.
            """
            split_format_str = '{%s}'
            sep = ':'
