...
from .cassconnect import (apippi_cursor, create_db, get_keyspace,
...
class TestCqlshOutput(BaseTestCase):
    ...
    def strip_read_repair_chance(self, describe_statement):
        ...
        Useful after apippi-13910, which removed read_repair_chance
    ...
    def test_empty_cf_output(self):
        # we print the header after apippi-6910
        self.assertQueriesGiveColoredOutput((
    ...
    def test_describe_schema_output(self):
        with testrun_cqlsh(tty=True, env=self.default_env) as c:
            for semicolon in ('', ';'):
                ...
                self.assertNoHasColors(output)
                # Since apippi-7622 'DESC FULL SCHEMA' also shows all VIRTUAL keyspaces
    ...
    def test_show_output(self):
        with testrun_cqlsh(tty=True, env=self.default_env) as c:
            ...
            self.assertRegex(output,
                    r'^\[cqlsh \S+ \| apippi \S+ \| CQL spec \S+ \| Native protocol \S+\]$')
