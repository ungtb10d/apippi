...
from apippi.metadata import maybe_escape_name as quote_name
...
def get_apippi_connection(cql_version=None):
...
def get_apippi_cursor(cql_version=None):
    return get_apippi_connection(cql_version=cql_version).cursor()
...
def create_db():
    with apippi_cursor(ks=None) as c:
        k = create_keyspace(c)
...
def apippi_connection(cql_version=None):
    """
    Make a apippi CQL connection with the given CQL version and get a cursor
    ...
    conn = get_apippi_connection(cql_version=cql_version)
    try:
        yield conn
    finally:
        conn.shutdown()
...
@contextlib.contextmanager
def apippi_cursor(cql_version=None, ks=''):
    """
    Make a apippi CQL connection with the given CQL version and get a cursor
    ...
    try:
        c = conn.connect(ks)
        # increase default timeout to fix flacky tests, see apippi-12481
