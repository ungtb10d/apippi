...
from .cassconnect import (get_apippi_connection, create_keyspace, remove_db, testrun_cqlsh)
...
class TestCqlshUnicode(BaseTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        s = get_apippi_connection().connect()
    ...
    def test_unicode_multiline_input(self):  # apippi-16400
        with testrun_cqlsh(tty=True, env=self.default_env) as c:
            value = '値'
    ...
    def test_unicode_desc(self):  # apippi-16539
        with testrun_cqlsh(tty=True, env=self.default_env) as c:
            v1 = 'ࠑ'
            v2 = 'Ξ'
    ...
    def test_unicode_esc(self):  # apippi-17617
