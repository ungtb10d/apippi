...
description = "CQL Shell for Apache apippi"
version = "6.2.0"
...
import apippi
from apippi.auth import PlainTextAuthProvider
from apippi.cluster import Cluster
from apippi.cqltypes import cql_typename
from apippi.marshal import int64_unpack
from apippi.metadata import (ColumnMetadata, KeyspaceMetadata, TableMetadata)
from apippi.policies import WhiteListRoundRobinPolicy
from apippi.query import SimpleStatement, ordered_dict_factory, TraceUnavailable
from apippi.util import datetime_from_timestamp
...
# The apippi distributed tests (dtests) also use this option in some tests when a well-known password is supplied via the command line.
...
def resolve_cql_history_file():
    default_cql_history = os.path.expanduser(os.path.join('~', '.apippi', 'cqlsh_history'))
...
DEFAULT_CQLSHRC = os.path.expanduser(os.path.join('~', '.apippi', 'cqlshrc'))
...
CQL_ERRORS = (
    apippi.AlreadyExists, apippi.AuthenticationFailed, apippi.CoordinationFailure,
    apippi.InvalidRequest, apippi.Timeout, apippi.Unauthorized, apippi.OperationTimedOut,
    apippi.cluster.NoHostAvailable,
    apippi.connection.ConnectionBusy, apippi.connection.ProtocolError, apippi.connection.ConnectionException,
    apippi.protocol.ErrorMessage, apippi.protocol.InternalError, apippi.query.TraceUnavailable
)
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=False,
        ...
        self.session.default_consistency_level = apippi.ConsistencyLevel.ONE
        ...
        self.consistency_level = apippi.ConsistencyLevel.ONE
        self.serial_consistency_level = apippi.ConsistencyLevel.SERIAL
    ...
    def show_version(self):
        ...
        print("[cqlsh %(shver)s | apippi %(build)s | CQL spec %(cql)s | Native protocol v%(protocol)s]" % vers)
    ...
    def get_fake_auth_table_meta(self, ksname, tablename):
        ...
        if tablename == 'roles':
            ...
            table_meta.columns['role'] = ColumnMetadata(table_meta, 'role', apippi.cqltypes.UTF8Type)
            table_meta.columns['is_superuser'] = ColumnMetadata(table_meta, 'is_superuser', apippi.cqltypes.BooleanType)
            table_meta.columns['can_login'] = ColumnMetadata(table_meta, 'can_login', apippi.cqltypes.BooleanType)
        elif tablename == 'role_permissions':
            ks_meta = KeyspaceMetadata(ksname, True, None, None)
            table_meta = TableMetadata(ks_meta, 'role_permissions')
            table_meta.columns['role'] = ColumnMetadata(table_meta, 'role', apippi.cqltypes.UTF8Type)
            table_meta.columns['resource'] = ColumnMetadata(table_meta, 'resource', apippi.cqltypes.UTF8Type)
            table_meta.columns['permission'] = ColumnMetadata(table_meta, 'permission', apippi.cqltypes.UTF8Type)
        else:
            raise ColumnFamilyNotFound("Column family {} not found".format(tablename))
    ...
    def perform_simple_statement(self, statement):
        ...
        # Even if statement failed we try to refresh schema if not agreed (see apippi-9689)
    ...
    def find_completions(self, text):
        ...
        return cqlruleset.cql_complete(stuff_to_complete, text, apippi_conn=self,
    ...
    def do_describe(self, parsed):
        ...
        (DESC may be used as a shorthand.)
          ...
          Outputs information about the connected apippi cluster, or about
        ...
        DESCRIBE CLUSTER
          ...
          Output information about the connected apippi cluster, such as the
          cluster name, and the partitioner and snitch in use. When you are
          connected to a non-system keyspace, also shows endpoint-range
          ownership information for the apippi ring.
        ...
        DESCRIBE <objname>
          ...
          view (in this order).
        """
        stmt = SimpleStatement(parsed.extract_orig(), consistency_level=apippi.ConsistencyLevel.LOCAL_ONE, fetch_size=self.page_size if self.use_paging else None)
        future = self.session.execute_async(stmt)

        if self.connection_versions['build'][0] < '4':
            print('\nWARN: DESCRIBE|DESC was moved to server side in apippi 4.0. As a consequence DESRIBE|DESC '
                  'will not work in cqlsh %r connected to apippi %r, the version that you are connected to. '
                  'DESCRIBE does not exist server side prior apippi 4.0.'
    ...
    def do_copy(self, parsed):
        ...
        COPY [cqlsh only]
          ...
          COPY x FROM: Imports CSV data into a apippi table
          COPY x TO: Exports data from a apippi table in CSV format.
    ...
    def do_show(self, parsed):
        ...
        SHOW VERSION
          ...
          Shows the version and build of the connected apippi instance, as
          well as the version of the CQL spec that the connected apippi
    ...
    def do_consistency(self, parsed):
        ...
        if level is None:
            print('Current consistency level is %s.' % (apippi.ConsistencyLevel.value_to_name[self.consistency_level]))
            return

        self.consistency_level = apippi.ConsistencyLevel.name_to_value[level.upper()]
    ...
    def do_serial(self, parsed):
        ...
        level = parsed.get_binding('level')
        if level is None:
            print('Current serial consistency level is %s.' % (apippi.ConsistencyLevel.value_to_name[self.serial_consistency_level]))
            return

        self.serial_consistency_level = apippi.ConsistencyLevel.name_to_value[level.upper()]
        ...
        for t in topics:
            if t.lower() in self.get_help_topics():
                ...
                if urlpart is not None:
                    url = "%s#%s" % (apippi_CQL_HTML, urlpart)
...
def read_options(cmdlineargs, environment=os.environ):
    ...
    #   cqlsh --credentials=~/.apippi/creds
    #   cqlsh --credentials ~/.apippi/creds
...
def setup_docspath(path):
    global apippi_CQL_HTML
    apippi_CQL_HTML_FALLBACK = 'https://apippi.apache.org/doc/latest/cql/index.html'
    #
    # default location of local CQL.html
    if os.path.exists(path + '/doc/cql3/CQL.html'):
        # default location of local CQL.html
        apippi_CQL_HTML = 'file://' + path + '/doc/cql3/CQL.html'
    elif os.path.exists('/usr/share/doc/apippi/CQL.html'):
        # fallback to package file
        apippi_CQL_HTML = 'file:///usr/share/doc/apippi/CQL.html'
    else:
        # fallback to online version
        apippi_CQL_HTML = apippi_CQL_HTML_FALLBACK
...
def insert_driver_hooks():
    ...
    # Display milliseconds when datetime overflows (apippi-10625), E.g., the year 10000.
    ...
    apippi.cqltypes.DateType.deserialize = staticmethod(deserialize_date_fallback_int)

    if hasattr(apippi, 'deserializers'):
        del apippi.deserializers.DesDateType

    # Return apippi.cqltypes.EMPTY instead of None for empty values
    apippi.cqltypes.apippiType.support_empty_values = True
