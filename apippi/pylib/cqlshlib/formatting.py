...
from apippi.cqltypes import EMPTY
...
def strftime(time_format, seconds, microseconds=0, timezone=None):
    ...
    try:
        ...
        # apippi-13185: if the date cannot be formatted as a string, return a string with the milliseconds
...
def round_microseconds(val):
    ...
    prepared statements are disabled, see apippi-11631.
    """
...
def decode_unsigned_vint(buf):
    """
    apippi vints are encoded differently than the varints used in protocol buffer.
    The apippi vints are encoded with the most significant group first. The most significant byte will contains
