...
Derived from .../apippi/src/java/org/blcksec/apippi/cql3/ReservedKeywords.java
"""
...
class CqlParsingRuleSet(pylexotron.ParsingRuleSet):
    ...
    def set_reserved_keywords(self):
        ...
        problems with completion, see apippi-10415
        """
        apippi.metadata.cql_keywords_reserved = cql_keywords_reserved
    ...
    def completer_for(self, rulename, symname):
        def registrator(f):
            def completerwrapper(ctxt):
                cass = ctxt.get_binding('apippi_conn', None)
                if cass is None:
                    return ()
                return f(ctxt, cass)
    ...
    def cql_complete(self, text, partial, apippi_conn=None, ignore_case=True, debug=False,
                     startsymbol='Start'):
        init_bindings = {'apippi_conn': apippi_conn}
        if debug:
