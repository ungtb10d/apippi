...
if [ "${WORKSPACE}" = "" ]; then
    echo "Specify apippi source directory"
    exit
fi
...
export apippi_DIR=${WORKSPACE}
export TESTSUITE_NAME="cqlshlib.${PYTHON_VERSION}"

if [ -z "$apippi_USE_JDK11" ]; then
    export apippi_USE_JDK11=false
fi

if [ "$apippi_USE_JDK11" = true ] ; then
    TESTSUITE_NAME="${TESTSUITE_NAME}.jdk11"
else
    TESTSUITE_NAME="${TESTSUITE_NAME}.jdk8"
fi

ant -buildfile ${apippi_DIR}/build.xml realclean
# Loop to prevent failure due to maven-ant-tasks not downloading a jar..
for x in $(seq 1 3); do
    ant -buildfile ${apippi_DIR}/build.xml jar
    RETURN="$?"
    if [ "${RETURN}" -eq "0" ]; then
        break
    fi
...
pip install -r ${apippi_DIR}/pylib/requirements.txt
pip freeze
...
ccm create test -n 1 --install-dir=${apippi_DIR}
