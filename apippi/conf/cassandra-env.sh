...
if [ $JAVA_VERSION -ge 11 ] ; then
    ...
    if [ "$?" = "1" ] ; then # [X] to prevent ccm from replacing this line
        ...
        mkdir -p ${apippi_LOG_DIR}
        JVM_OPTS="$JVM_OPTS -Xlog:gc=info,heap*=trace,age*=debug,safepoint=info,promotion*=trace:file=${apippi_LOG_DIR}/gc.log:time,uptime,pid,tid,level:filecount=10,filesize=10485760"
    fi
else
    # Java 8
    ...
    if [ "$?" = "1" ] ; then # [X] to prevent ccm from replacing this line
        # only add -Xlog:gc if it's not mentioned in jvm-server.options file
        mkdir -p ${apippi_LOG_DIR}
        JVM_OPTS="$JVM_OPTS -Xloggc:${apippi_LOG_DIR}/gc.log"
    fi
fi
...
if [ "x$MAX_HEAP_SIZE" = "x" ] && [ "x$HEAP_NEWSIZE" = "x" -o $USING_G1 -eq 0 ]; then
    ...
    echo "please set or unset MAX_HEAP_SIZE and HEAP_NEWSIZE in pairs when using CMS GC (see apippi-env.sh)"
    exit 1
fi
...
JVM_OPTS="$JVM_OPTS -XX:CompileCommandFile=$apippi_CONF/hotspot_compiler"

# add the jamm javaagent
JVM_OPTS="$JVM_OPTS -javaagent:$apippi_HOME/lib/jamm-0.3.2.jar"

# set jvm HeapDumpPath with apippi_HEAPDUMP_DIR
if [ "x$apippi_HEAPDUMP_DIR" != "x" ]; then
    JVM_OPTS="$JVM_OPTS -XX:HeapDumpPath=$apippi_HEAPDUMP_DIR/apippi-`date +%s`-pid$$.hprof"
fi
...
# JVM_OPTS="$JVM_OPTS -Dapippi.printHeapHistogramOnOutOfMemoryError=true"
...
#
# apippi ships with JMX accessible *only* from localhost.  
# To enable remote JMX connections, uncomment lines below
# with authentication and/or ssl enabled. See https://wiki.apache.org/apippi/JmxSecurity 
#
if [ "x$LOCAL_JMX" = "x" ]; then
    LOCAL_JMX=yes
fi

# Specifies the default port over which apippi will be available for
# JMX connections.
# For security reasons, you should not expose this port to the internet.  Firewall it if needed.
JMX_PORT="7199"

if [ "$LOCAL_JMX" = "yes" ]; then
  JVM_OPTS="$JVM_OPTS -Dapippi.jmx.local.port=$JMX_PORT"
  JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.authenticate=false"
else
  JVM_OPTS="$JVM_OPTS -Dapippi.jmx.remote.port=$JMX_PORT"
  # if ssl is enabled the same port cannot be used for both jmx and rmi so either
  # pick another value for this property or comment out to use a random port (though see apippi-7087 for origins)
...
JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.password.file=/etc/apippi/jmxremote.password"
#JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.access.file=/etc/apippi/jmxremote.access"
...
## apippi ships with a LoginModule implementation - org.apache.apippi.auth.apippiLoginModule -
## which delegates to the IAuthenticator configured in apippi.yaml. See the sample JAAS configuration
## file apippi-jaas.config
#JVM_OPTS="$JVM_OPTS -Dapippi.jmx.remote.login.config=apippiLogin"
#JVM_OPTS="$JVM_OPTS -Djava.security.auth.login.config=$apippi_CONF/apippi-jaas.config"

## apippi also ships with a helper for delegating JMX authz calls to the configured IAuthorizer,
## uncomment this to use it. Requires one of the two authentication options to be enabled
#JVM_OPTS="$JVM_OPTS -Dapippi.jmx.authorizer=org.apache.apippi.auth.jmx.AuthorizationProxy"

# To use mx4j, an HTML interface for JMX, add mx4j-tools.jar to the lib/
# directory.
# See http://apippi.apache.org/doc/latest/operating/metrics.html#jmx
# By default mx4j listens on the broadcast_address, port 8081. Uncomment the following lines
# to control its listen address and port.
#MX4J_ADDRESS="127.0.0.1"
#MX4J_PORT="8081"

# apippi uses SIGAR to capture OS metrics apippi-7838
# for SIGAR we have to set the java.library.path
# to the location of the native libraries.
JVM_OPTS="$JVM_OPTS -Djava.library.path=$apippi_HOME/lib/sigar-bin"
