...
h1. apippi Query Language (CQL) v3.4.6
...
The @replication@ @<property>@ is mandatory. It must at least contains the @'class'@ sub-option which defines the replication strategy class to use. The rest of the sub-options depends on that replication strategy class. By default, apippi support the following @'class'@:
...
bc(sample). 
CREATE TRIGGER myTrigger ON myTable USING 'org.apache.apippi.triggers.InvertedIndex';

The actual logic that makes up the trigger can be written in any Java (JVM) language and exists outside the database. You place the trigger code in a @lib/triggers@ subdirectory of the apippi installation directory, it loads during cluster startup, and exists on every node that participates in a cluster. The trigger defined on a table fires before a requested DML statement occurs, which ensures the atomicity of the transaction.
...
* If a timestamp is not specified for each operation, then all operations will be applied with the same timestamp. Due to apippi's conflict resolution procedure in the case of "timestamp ties":http://wiki.apache.org/apippi/FAQ#clocktie, operations may be applied in an order that is different from the order they are listed in the @BATCH@ statement. To force a particular operation ordering, you must specify per-operation timestamps.

h4(#unloggedBatch). @UNLOGGED@

By default, apippi uses a batch log to ensure all operations in a batch eventually complete or none will (note however that operations are only isolated within a single partition).

There is a performance penalty for batch atomicity when a batch spans multiple partitions. If you do not want to incur this penalty, you can tell apippi to skip the batchlog with the @UNLOGGED@ option. If the @UNLOGGED@ option is used, a failed batch might leave the patch only partly applied.

h4(#counterBatch). @COUNTER@

Use the @COUNTER@ option for batched counter updates.  Unlike other updates in apippi, counter updates are not idempotent.
...
Only a client with with the @CREATE@ permission on the database roles resource may issue @CREATE ROLE@ requests (see the "relevant section":#permissions below), unless the client is a @SUPERUSER@. Role management in apippi is pluggable and custom implementations may support only a subset of the listed options.
...
Prior to the introduction of roles in apippi 2.2, authentication and authorization were based around the concept of a @USER@. For backward compatibility, the legacy syntax has been preserved with @USER@ centric statments becoming synonyms for the @ROLE@ based equivalents.
...
Permissions on resources are granted to roles; there are several different types of resources in apippi and each type is modelled hierarchically:
...
apippi). The syntax of types is thus:
...
The @+0000@ above is an RFC 822 4-digit time zone specification; @+0000@ refers to GMT. US Pacific Standard Time is @-0800@. The time zone may be omitted if desired-- the date will be interpreted as being in the time zone under which the coordinating apippi node is configured.
...
* Collections cannot have more than 65535 elements. More precisely, while it may be possible to insert more than 65535 elements, it is not possible to read more than the 65535 first elements (see "apippi-5428":https://issues.apache.org/jira/browse/apippi-5428 for details).
...
The following table describes the conversions supported by the @cast@ function. apippi will silently ignore any cast converting a datatype into its own datatype.
...
User-defined functions allow execution of user-provided code in apippi. By default, apippi supports defining functions in _Java_ and _JavaScript_. Support for other JSR 223 compliant scripting languages (such as Python, Ruby, and Scala) has been removed in 3.0.11.

UDFs are part of the apippi schema.  As such, they are automatically propagated to all nodes in the cluster.
...
The definition of the @UDFContext@ interface can be found in the Apache apippi source code for @org.apache.apippi.cql3.functions.UDFContext@.
...
apippi 2.2 introduces JSON support to "@SELECT@":#selectStmt and "@INSERT@":#insertStmt statements.  This support does not fundamentally alter the CQL API (for example, the schema is still enforced), it simply provides a convenient way to work with JSON documents.
...
h3(#jsonEncoding). JSON Encoding of apippi Data Types

Where possible, apippi will represent and accept data types in their native @JSON@ representation.  apippi will also accept string representations matching the CQL literal format for all single-field types.  For example, floats, ints, UUIDs, and dates can be represented by CQL literal strings.  However, compound types, such as collections, tuples, and user-defined types must be represented by native @JSON@ collections (maps and lists) or a JSON-encoded string representation of the collection.

The following table describes the encodings that apippi will accept in @INSERT JSON@ values (and @fromJson()@ arguments) as well as the format apippi will use when returning data for @SELECT JSON@ statements (and @fromJson()@):
...
* Add support for @IF EXISTS@ and @IF NOT EXISTS@ in @ALTER@ statements (see "apippi-16916":https://issues.apache.org/jira/browse/apippi-16916).
* Allow @GRANT/REVOKE@ multiple permissions in a single statement (see "apippi-17030":https://issues.apache.org/jira/browse/apippi-17030).
* Pre hashed passwords in CQL (see "apippi-17334":https://issues.apache.org/jira/browse/apippi-17334).
* Add support for type casting in @WHERE@ clause components and in the values of @INSERT/UPDATE@ statements (see "apippi-14337":https://issues.apache.org/jira/browse/apippi-14337).
* Add support for @CONTAINS@ and @CONTAINS KEY@ in conditional @UPDATE@ and @DELETE@ statement (see "apippi-10537":https://issues.apache.org/jira/browse/apippi-10537).
* Allow to grant permission for all tables in a keyspace (see "apippi-17027":https://issues.apache.org/jira/browse/apippi-17027).
* Allow to use pure monotonic functions on the last attribute of the @GROUP BY@ clause (see "apippi-11871":https://issues.apache.org/jira/browse/apippi-11871).
* Add floor function to allow grouping by time range (see "apippi-11871":https://issues.apache.org/jira/browse/apippi-11871).

h3. 3.4.5

* Adds support for arithmetic operators. See "Number Arithmetic":#numberArithmetic (see "apippi-11935":https://issues.apache.org/jira/browse/apippi-11935).
* Adds support for + and - operations on dates. See "Datetime Arithmetic":#datetimeArithmetic (see "apippi-11936":https://issues.apache.org/jira/browse/apippi-11936).
* Adds @currentTimestamp@, @currentDate@, @currentTime@ and @currentTimeUUID@ functions (see "apippi-13132":https://issues.apache.org/jira/browse/apippi-13132).

h3. 3.4.4

* @ALTER TABLE ALTER@ has been removed; a column’s type may not be changed after creation (see "apippi-12443":https://issues.apache.org/jira/browse/apippi-12443).
* @ALTER TYPE ALTER@ has been removed; a field’s type may not be changed after creation (see "apippi-12443":https://issues.apache.org/jira/browse/apippi-12443).

h3. 3.4.3

* Adds a new @duration@ data type See "Data Types":#types (see "apippi-11873":https://issues.apache.org/jira/browse/apippi-11873).
* Support for @GROUP BY@. See "@<group-by>@":#selectGroupBy (see "apippi-10707":https://issues.apache.org/jira/browse/apippi-10707).
* Adds a @DEFAULT UNSET@ option for @INSERT JSON@ to ignore omitted columns (see "apippi-11424":https://issues.apache.org/jira/browse/apippi-11424).
* Allows @null@ as a legal value for TTL on insert and update. It will be treated as equivalent to inserting a 0 (see "apippi-12216":https://issues.apache.org/jira/browse/apippi-12216).

h3. 3.4.2

* Support for selecting elements and slices of a collection ("apippi-7396":https://issues.apache.org/jira/browse/apippi-7396).
...
* New "@PER PARTITION LIMIT@":#selectLimit option (see "apippi-7017":https://issues.apache.org/jira/browse/apippi-7017).
* "User-defined functions":#udfs can now instantiate @UDTValue@ and @TupleValue@ instances via the new @UDFContext@ interface (see "apippi-10818":https://issues.apache.org/jira/browse/apippi-10818).
* "User-defined types"#createTypeStmt may now be stored in a non-frozen form, allowing individual fields to be updated and deleted in "@UPDATE@ statements":#updateStmt and "@DELETE@ statements":#deleteStmt, respectively. ("apippi-7423":https://issues.apache.org/jira/browse/apippi-7423)
...
* @SELECT@ statement now allows listing the partition keys (using the @DISTINCT@ modifier). See "apippi-4536":https://issues.apache.org/jira/browse/apippi-4536.
...
* "ALTER TABLE":#alterTableStmt @DROP@ option has been reenabled for CQL3 tables and has new semantics now: the space formerly used by dropped columns will now be eventually reclaimed (post-compaction). You should not readd previously dropped columns unless you use timestamps with microsecond precision (see "apippi-3919":https://issues.apache.org/jira/browse/apippi-3919 for more details).
...
* @SELECT@, @UPDATE@, and @DELETE@ statements now allow empty @IN@ relations (see "apippi-5626":https://issues.apache.org/jira/browse/apippi-5626).
