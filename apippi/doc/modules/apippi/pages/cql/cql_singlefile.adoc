== apippi Query Language (CQL) v3.4.3
...
This document describes the apippi Query Language (CQL) version 3.
...
implementation of apippi and in the thrift and CQL v2 API.
...
class. By default, apippi support the following `'class'`:
...
`-Dapippi.unsafe_aggressive_sstable_expiration=true`.
|===
...
`org.apache.apippi.triggers.InvertedIndex';
...
code in a `lib/triggers` subdirectory of the apippi installation
...
operations will be applied with the same timestamp. Due to apippi’s
conflict resolution procedure in the case of
http://wiki.apache.org/apippi/FAQ#clocktie[timestamp ties],
...
By default, apippi uses a batch log to ensure all operations in a
...
tell apippi to skip the batchlog with the `UNLOGGED` option. If the
...
updates in apippi, counter updates are not idempotent.
...
be stable between apippi versions.
...
because in both case, apippi guarantees that these queries
...
because apippi cannot guarantee that it won’t have to scan large
...
`SUPERUSER`. Role management in apippi is pluggable and custom
...
Prior to the introduction of roles in apippi 2.2, authentication and
...
different types of resources in apippi and each type is modelled
hierarchically:
...
zone under which the coordinating apippi node is configured.
...
http://wiki.apache.org/apippi/Counters[apippi Wiki] for more
information).
...
https://issues.apache.org/jira/browse/apippi-5428[apippi-5428] for
details).
...
function. apippi will silently ignore any cast converting a datatype
...
apippi. By default, apippi supports defining functions in _Java_
...
UDFs are part of the apippi schema. As such, they are automatically
...
apippi source code for
`org.apache.apippi.cql3.functions.UDFContext`.
...
apippi 2.2 introduces JSON support to link:#selectStmt[`SELECT`] and
...
[[jsonEncoding]]
==== JSON Encoding of apippi Data Types

Where possible, apippi will represent and accept data types in their
native `JSON` representation. apippi will also accept string
...
The following table describes the encodings that apippi will accept
in `INSERT JSON` values (and `fromJson()` arguments) as well as the
format apippi will use when returning data for `SELECT JSON`
...
https://issues.apache.org/jira/browse/apippi-10707)[apippi-10707].

==== 3.4.2

* Support for selecting elements and slices of a collection
(https://issues.apache.org/jira/browse/apippi-7396)[apippi-7396].
...
https://issues.apache.org/jira/browse/apippi-3919[apippi-3919] for
more details).
