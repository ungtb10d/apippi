...
By default, apippi supports the following `'class'` values:
...
Within a table, CQL defines the notion of a _partition_ that defines the location of data within a apippi cluster.
...
node in the cluster. 
The localization of data is important to the efficient retrieval of data, requiring the apippi coordinator
...
| `memtable_flush_period_in_ms` |_simple_ |0 |Time (in ms) before apippi flushes memtables to disk
...
By default, apippi read coordinators only query as many replicas as
...
Pre-apippi 4.0 speculative retry policy takes a single string as a parameter:
...
apippi 4.0 supports case-insensitivity for speculative retry values (https://issues.apache.org/jira/browse/apippi-14293[apippi-14293]). For example, assigning the value as `none`, `None`, or `NONE` has the same effect.
...
apippi 4.0 adds support for hybrid `MIN()` and `MAX()` speculative retry policies, with a mix and match of either `MIN(), MAX()`, `MIN(), MIN()`, or `MAX(), MAX()` (https://issues.apache.org/jira/browse/apippi-14293[apippi-14293]).
...
| `crc_check_chance` | 1.0 | Determines how likely apippi is to verify the checksum on each
...
apippi allows the client to provide any timestamp on any table, it is
