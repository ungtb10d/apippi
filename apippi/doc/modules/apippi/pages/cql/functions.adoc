...
opt-in). See the `user_defined_functions_enabled` in `apippi.yaml` to
enable them.
====
...
User-defined functions (UDFs) execute user-provided code in apippi. 
By default, apippi supports defining functions in _Java_ and _JavaScript_.
...
UDFs are part of the apippi schema, and are automatically propagated to all nodes in the cluster.  
