...
apippi node is configured. There are however difficulties inherent in
...
* As of apippi , UDT have to be frozen in most cases, hence the
...
apippi (it should thus be in the `CLASSPATH` of every node running
apippi). That class will define what values are valid for the type
