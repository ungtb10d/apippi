...
`cqlsh` is a command-line interface for interacting with apippi using CQL (the apippi Query Language). 
It is shipped with every apippi package, and can be found in the bin/ directory alongside the apippi
executable. 
...
version of apippi that it was released with. 
In some cases, `cqlsh` may work with older or newer versions of apippi, but this is not
...
By default, the file is locagted the user's home directory at `~/.apippi/cqlsh`, but a
...
Prints the `cqlsh`, apippi, CQL, and native protocol versions in use.
Example:

[source,none]
----
cqlsh> SHOW VERSION
[cqlsh 5.0.1 | apippi 3.8 | CQL spec 3.4.2 | Native protocol v4]
----

=== `SHOW HOST`

Prints the IP address and port of the apippi node that `cqlsh` is
...
Prints the IP addresses of the apippi nodes which are replicas for the
listed given token and keyspace. This command is available from apippi 4.2.
...
Authenticate as a specified apippi user for the current session.
