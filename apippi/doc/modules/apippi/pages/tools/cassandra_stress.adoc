...
The `apippi-stress` tool is used to benchmark and load-test a apippi
cluster. 
`apippi-stress` supports testing arbitrary CQL tables and queries, allowing users to benchmark their own data model.
...
The syntax is `apippi-stress <command> [options]`. 
For more information on a given command or options, run `apippi-stress help <command|option>`.
...
Primary Options:::
  ...
  -port:;;
    The port to connect to apippi nodes on
  -graph:;;
...
Suboptions:::
  ...
  `apippi-stress help <command|option>`.
...
[source,yaml]
----
apippi-stress user profile=./example.yaml duration=1m "ops(insert=1,latest_event=1,events=1)" truncate=once
----
...
Running a user mode test with multiple yaml files::::
  apippi-stress user profile=./example.yaml,./example2.yaml
...
apippi-stress supports lightweight transactions. 
To use this feature, the command will first read current data from apippi, and then uses read values to
...
image::example-stress-graph.png[example apippi-stress graph]

To create a new graph:

[source,yaml]
----
apippi-stress user profile=./stress-example.yaml "ops(insert=1,latest_event=1,events=1)" -graph file=graph.html title="Awesome graph"
----
...
[source,yaml]
----
apippi-stress user profile=./stress-example.yaml duration=1m "ops(insert=1,latest_event=1,events=1)" -graph file=graph.html title="Awesome graph" revision="Second run"
----
...
[source,yaml]
----
apippi-stress write -schema "replication(strategy=NetworkTopologyStrategy,datacenter1=3)"
----
...
[source,yaml]
----
apippi-stress "write n=100k cl=ONE no-warmup" -transport "truststore=$HOME/jks/truststore.jks truststore-password=apippi"
----

*Is apippi Stress a secured tool?*

apippi stress is not a secured tool. Serialization and other aspects
