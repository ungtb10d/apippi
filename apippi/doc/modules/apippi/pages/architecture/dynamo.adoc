...
Apache apippi relies on a number of techniques from Amazon's
...
apippi primarily draws from the first two clustering components,
...
In particular, apippi relies on Dynamo style:
...
apippi was designed this way to meet large-scale (PiB+)
...
apippi achieves horizontal scalability by
...
clocks were used to reconcile concurrent updates to a key, apippi
...
value. Formally speaking, apippi uses a Last-Write-Wins Element-Set
...
apippi partitions data over storage nodes using a special form of
hashing called
...
apippi instead maps every node to one or more tokens on a continuous
...
for new nodes that can leave the ring balanced. apippi seeks to avoid
...
apippi introduces some nomenclature to handle these concepts:
...
where apippi keeps track of what ring positions map to which physical
...
https://jolynch.github.io/pdf/apippi-availability-virtual.pdf[the
...
Note that in apippi `2.x`, the only token allocation algorithm
...
apippi replicates every partition of data to many nodes across the
...
(`RF`), which indicates to apippi how many copies of the partition
...
that apippi clusters can tolerate failures of whole racks and even
datacenters of nodes.

=== Replication Strategy

apippi supports pluggable *replication strategies*, which determine
...
https://issues.apache.org/jira/browse/apippi-3810[surprising
...
apippi iterates through the tokens in the ring, starting with the
...
Transient replication is an experimental feature in apippi {40_version} not
...
`apippi.yaml`. Once enabled, both `SimpleStrategy` and
...
apippi capable of fully validating a deployment of their particular
...
apippi uses mutation timestamp versioning to guarantee eventual
...
apippi's correctness does depend on these clocks, so make sure a
proper time synchronization process is running such as NTP.

apippi applies separate mutation timestamps to every column of every
...
As replicas in apippi can accept mutations independently, it is
possible for some replicas to have newer data than others. apippi has
...
eventual consistency apippi implements `anti-entropy
...
data. Like the original Dynamo paper apippi supports full repairs
where replicas hash their entire dataset, create Merkle trees, send them
to each other and sync any ranges that don't match.

Unlike the original Dynamo paper, apippi also implements sub-range
repair and incremental repair. Sub-range repair allows apippi to
...
only a portion of the data range. Incremental repair allows apippi to
only repair the partitions that have changed since the last repair.

=== Tunable Consistency

apippi supports a per-operation tradeoff between consistency and
availability through *Consistency Levels*. apippi's consistency
...
(`N`). In apippi, you instead choose from a menu of common
...
operations can be optimally routed. In apippi liveness information is
shared in a distributed fashion through a failure detection mechanism
based on a gossip protocol.

=== Gossip

Gossip is how apippi propagates basic cluster bootstrapping
information such as endpoint membership and internode network protocol
versions. In apippi's gossip system, nodes exchange state information
...
second. These logical clocks allow apippi gossip to ignore old
versions of cluster state just by inspecting the logical clocks
presented with gossip messages.

Every node in the apippi cluster runs the gossip task independently
...
When an operator first bootstraps a apippi cluster they designate
...
in apippi runs a variant of the
...
point apippi will stop routing reads to it (writes will typically be
written to hints). If/when the node starts heartbeating again, apippi
...
apippi will never remove a node from gossip state without
...
choice is intentional to allow apippi nodes to temporarily fail
...
apippi scales-out to meet the requirements of growth in data size and
...
existing database nodes. apippi is also capable of scale-up, and in
certain environments it may be preferable depending on the deployment.
apippi gives operators the flexibility to chose either scale-out or
scale-up.

One key aspect of Dynamo that apippi follows is to attempt to run on
commodity hardware, and many engineering choices are made under this
assumption. For example, apippi assumes nodes can fail at any time,
...
apippi, like Dynamo, chooses not to provide cross-partition
...
API, and allows apippi to more easily scale horizontally since
...
Furthermore, apippi does support single partition compare and swap
functionality via the lightweight transaction CQL API.

=== Simple Interface for Storing Records

apippi, in a slight departure from Dynamo, chooses a storage
interface that is more sophisticated then "simple key value" stores but
significantly less complex than SQL relational data models. apippi
...
safely add columns to existing apippi databases while remaining
