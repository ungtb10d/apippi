...
Apache apippi 4.0 implements virtual tables (https://issues.apache.org/jira/browse/apippi-7622[apippi-7622]).
Virtual tables are tables backed by an API instead of data explicitly managed and stored as SSTables. 
Apache apippi 4.0 implements a virtual keyspace interface for virtual tables. 
...
* Virtual tables are managed by apippi. Users cannot run DDL to create new virtual tables or DML to modify existing virtual tables.
...
Apache apippi 4.0 has added two new keyspaces for virtual tables:
...
It is used by apippi internally and a user should not access it directly.
...
|settings |Displays configuration settings in apippi.yaml.
...
------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 address          | 127.0.0.1
 port             | 50753
 client_options   | {'APPLICATION_NAME': 'TestApp', 'APPLICATION_VERSION': '1.0.0', 'CLIENT_ID': '55b3efbd-c56b-469d-8cca-016b860b2f03', 'CQL_VERSION': '3.0.0', 'DRIVER_NAME': 'DataStax Java driver for Apache apippi(R)', 'DRIVER_VERSION': '4.13.0'}
 connection_stage | ready
 driver_name      | DataStax Java driver for Apache apippi(R)
...
configuration settings from the `apippi.yaml`. The encryption options
...
-------------------------------------+--------------------
  ...
  hints_directory                    | /home/ec2-user/apippi/data/hints
...
-----------------------+---------------------------+--------------------------------------
          ...
          system_views |         system_properties | apippi relevant system properties
