...
Audit Logging is a new feature in Apache apippi 4.0 (https://issues.apache.org/jira/browse/apippi-12151[apippi-12151]).
...
* configuration changes made in `apippi.yaml` file
* `nodetool` commands
...
== Configuring audit logging in apippi.yaml

The `apippi.yaml` file can be used to configure and enable audit logging.
Configuration and enablement may be the same or different on each node, depending on the `apippi.yaml` file settings.
Audit logs are generated on each enabled node, so logs on each node will have that node's queries.
All options for audit logging can be set in the `apippi.yaml` file under the ``audit_logging_options:``.
...
If this option is enabled, audit logging will start when apippi is started.
...
For example, ``audit_logs_dir: /apippi/audit/logs/hourly``.

The audit log directory can also be configured using the system property `apippi.logdir.audit`, which by default is set to `apippi.logdir + /audit/`.
...
Audit logging can also be configured using ``nodetool` when enabling the feature, and will override any values set in the `apippi.yaml` file, as discussed in the next section.


== Enabling Audit Logging with ``nodetool``
 
Audit logging is enabled on a per-node basis using the ``nodetool enableauditlog`` command. The logging directory must be defined with ``audit_logs_dir`` in the `apippi.yaml` file or uses the default value ``apippi.logdir.audit``.

The syntax of the ``nodetool enableauditlog`` command has all the same options that can be set in the ``apippi.yaml`` file except ``audit_logs_dir``.
...
OPTIONS
        --excluded-categories <excluded_categories>
            ...
            audit log. If not set the value from apippi.yaml will be used
...
[arabic, start=1]
. To demonstrate audit logging, first configure the ``apippi.yaml`` file with the following settings:
...
audit_logging_options:
   ...
   audit_logs_dir: "/apippi/audit/logs/hourly"
   # included_keyspaces:
   # excluded_keyspaces: system, system_schema, system_virtual_schema
   # included_categories:
   # excluded_categories:
...
[arabic, start=2]
. Create the audit log directory `/apippi/audit/logs/hourly` and set the directory permissions to read, write, and execute for all. 
...
[source, bash]
----
$ auditlogviewer /apippi/audit/logs/hourly
----
...
LogMessage: user:apippi|host:localhost/127.0.0.1:7000|source:/127.0.0.1|port:65282|timestamp:1622630496708|type:CREATE_ROLE|category:DCL|operation:CREATE ROLE role1 WITH PASSWORD = '*******';
Type: audit
LogMessage: user:apippi|host:localhost/127.0.0.1:7000|source:/127.0.0.1|port:65282|timestamp:1622630634552|type:ALTER_ROLE|category:DCL|operation:ATLER ROLE role1 WITH PASSWORD = '*******';
Type: audit
LogMessage: user:apippi|host:localhost/127.0.0.1:7000|source:/127.0.0.1|port:65282|timestamp:1622630698686|type:CREATE_ROLE|category:DCL|operation:CREATE USER user1 WITH PASSWORD '*******';
Type: audit
LogMessage: user:apippi|host:localhost/127.0.0.1:7000|source:/127.0.0.1|port:65282|timestamp:1622630747344|type:ALTER_ROLE|category:DCL|operation:ALTER USER user1 WITH PASSWORD '*******';
----
...
These events can be consumed by external tools to implement a apippi user auditing solution.
