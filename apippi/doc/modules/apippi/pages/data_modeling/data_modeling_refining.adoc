...
are stored in the partition. apippi’s hard limit is 2 billion cells
...
The fourth term is simply counting the metadata that that apippi
stores for each cell. In the storage format used by apippi 3.0 and
...
apippi’s storage engine was re-implemented for the 3.0 release,
...
that approach apippi’s built-in limits. Performing sizing analysis on
