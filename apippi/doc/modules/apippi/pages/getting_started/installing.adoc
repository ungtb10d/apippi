...
Apache apippi on Linux servers.

apippi runs on a wide array of Linux distributions including (but not
limited to):
...
`java -version`.
* *NOTE*: Experimental support for Java 11 was added in apippi {40_version}
(https://issues.apache.org/jira/browse/apippi-9608[apippi-9608]).
Full support is effective apippi 4.0.2 version (https://issues.apache.org/jira/browse/apippi-16894[apippi-16894])
For more information, see
https://github.com/blcksec/apippi/blob/trunk/NEWS.txt[NEWS.txt].
...
There are three methods of installing apippi that are common:
...
Pull the appropriate image and then start apippi with a run command. 
...
install apippi using YUM. 
Install the Debian build on Ubuntu and other Debian-based
distributions if you want to install apippi using APT. 
Note that both the YUM and APT methods required `root` permissions and 
will install the binaries and configuration files as the `apippi` OS user.
...
Apache apippi image available from the https://hub.docker.com/_/apippi[Dockerhub].

[arabic, start=2]
. Start apippi with a `docker run` command:
...
The `--name` option will be the name of the apippi cluster created.

[arabic, start=3]
. Start the CQL shell, `cqlsh` to interact with the apippi node created:
...
{cass_url}download/[Apache apippi Download] site.
For example, to download apippi {40_version}:
...
release. To download an earlier version of apippi, visit the
http://archive.apache.org/dist/apippi/[Apache Archives].
...
The files will be extracted to the `apache-apippi-4.0.0/` directory.
...
----
<1> location of the commands to run apippi, cqlsh, nodetool, and SSTable tools
<2> location of apippi.yaml and other configuration files
<3> location of the commit logs, hints, and SSTables
<4> location of system and debug logs
<5>location of apippi-stress tool
...
NOTE: This will run apippi as the authenticated Linux user.
...
[arabic, start=2]
. Add the Apache repository of apippi to the file
`apippi.sources.list`. 
...
[arabic, start=3]
. Add the Apache apippi repository keys to the list of trusted keys
on the server:
...
[arabic, start=2]
. Add the Apache repository of apippi to the file
`/etc/yum.repos.d/apippi.repo` (as the `root` user).
include::partial$package_versions.adoc[]
...
NOTE: A new Linux user `apippi` will get created as part of the
installation. The apippi service will also be run as this user.

[arabic, start=5]
. Start the apippi service:
