...
Apache apippi stores data in immutable SSTable files. Backups in
Apache apippi database are backup copies of the database data that is
...
Apache apippi supports two kinds of backup strategies.
...
The setting `snapshot_before_compaction` in the `apippi.yaml` file determines if
...
setting `auto_snapshot` to true (default) in `apippi.yaml`. 
Truncates could be delayed due to the auto snapshots and another setting in
`apippi.yaml` determines how long the coordinator should wait for
truncates to complete. 
By default apippi waits 60 seconds for auto snapshots to complete.
...
Incremental backups are not enabled by default and must be enabled explicitly in `apippi.yaml` (with
`incremental_backups` setting) or with `nodetool`. 
Once enabled, apippi creates a hard link to each SSTable flushed or streamed
locally in a `backups/` subdirectory of the keyspace data. 
Incremental backups of system tables are also created.

== Data Directory Structure

The directory structure of apippi data consists of different
...
the table directory. 
The directory structure for apippi is illustrated in Figure 1.

image::Figure_1_backups.jpg[Data directory structure for backups]

Figure 1. Directory Structure for apippi Data
...
We have used a three node apippi cluster. 
...
have set `auto_snapshots` setting to `false` in the `apippi.yaml` file:
...
Incremental backups are created within the apippi’s `data` directory
