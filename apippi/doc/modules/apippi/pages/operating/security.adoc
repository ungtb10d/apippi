...
By default, these features are disabled as apippi is configured to
easily find and be found by other members of a cluster. In other words,
an out-of-the-box apippi installation presents a large attack surface
...
theses vectors. Therefore, understanding apippi's security features
is crucial to configuring your cluster to meet your security needs.

== TLS/SSL Encryption

apippi provides secure communication between a client machine and a
...
the settings in `apippi.yaml`, but this is not recommended unless
...
not involve changing encryption settings in apippi.yaml. See
https://docs.oracle.com/javase/8/docs/technotes/guides/security/jsse/FIPS.html[the
java document on FIPS] for more details.

apippi provides flexibility of using Java based key material or
...
https://github.com/blcksec/apippi/blob/trunk/src/java/org/blcksec/apippi/security/ISslContextFactory.java[ISslContextCreationFactory]
...
appropriately. See https://github.com/blcksec/apippi/tree/trunk/examples/ssl-factory[ssl-factory examples]
for details. Refer to the below class diagram to understand the
class hierarchy.

image::apippi_ssl_context_factory_pem.png[image]
...
Beginning with apippi 4, apippi supports hot reloading of SSL
Certificates. If SSL/TLS support is enabled in apippi and you are
using default file based key material, the node periodically (every
10 minutes) polls the Trust and Key Stores specified in
apippi.yaml. When the files are updated, apippi will reload
...
setting, apippi polls (at the same periodic interval mentioned above)
your implementation to check if the SSL certificates need to be
reloaded. See the https://github.com/blcksec/apippi/blob/trunk/src/java/org/blcksec/apippi/security/ISslContextFactory.java#L90[ISslContextFactory] documentation for more details.
If you are using one of the apippi's in-built SSL context factory
...
`nodetool reloadssl` command. Use this if you want to apippi to
immediately notice the changed certificates.

=== Inter-node Encryption

The settings for managing inter-node encryption are found in
`apippi.yaml` in the `server_encryption_options` section. To enable
...
`apippi.yaml` in the `client_encryption_options` section. There are
...
`native_transport_port_ssl` setting in `apippi.yaml` to specify the
port to be used for secure client communication.

[[operation-roles]]
== Roles

apippi uses database roles, which may represent either a single user
or a group of users, in both authentication and permissions management.
Role management is an extension point in apippi and may be configured
using the `role_manager` setting in `apippi.yaml`. The default
setting uses `apippiRoleManager`, an implementation which stores role
...
Authentication is pluggable in apippi and is configured using the
`authenticator` setting in `apippi.yaml`. apippi ships with two
options included in the default distribution.

By default, apippi is configured with `AllowAllAuthenticator` which
...
authentication is a necessary condition of apippi's permissions
...
[arabic, start=2]
. Edit `apippi.yaml` to change the `authenticator` option like so:
...
[source,cql]
----
ALTER ROLE apippi WITH SUPERUSER = false AND LOGIN = false;
----
...
xref:cql/security.adoc#operation-roles[`apippiRoleManager`].
...
Authorization is pluggable in apippi and is configured using the
`authorizer` setting in `apippi.yaml`. apippi ships with two
options included in the default distribution.

By default, apippi is configured with `AllowAllAuthorizer` which
...
The default distribution also includes `apippiAuthorizer`, which does
implement full permissions management functionality and stores its data
in apippi system tables.
...
[arabic]
. On the selected node, edit `apippi.yaml` to change the `authorizer`
option like so:

[source,yaml]
----
authorizer: apippiAuthorizer
----
...
be configured (and even disabled) from `apippi.yaml` or using a JMX
...
re-read from `apippi.yaml` when the node is restarted.
...
The naming for these options in `apippi.yaml` follows the convention:
...
more closely with apippi's own auth subsystem.

The default settings for apippi make JMX accessible only from
localhost. To enable remote JMX connections, edit `apippi-env.sh`
...
text file. The location of this file is set in `apippi-env.sh` by the
line:
...
Secure the credentials file so that only the user running the apippi
process can read it :

[source,bash]
----
$ chown apippi:apippi /etc/apippi/jmxremote.password
$ chmod 400 /etc/apippi/jmxremote.password
----
...
this context as most operational tools in apippi require full
read/write access. To configure a simple access file, uncomment this
line in `apippi-env.sh`:
...
apippi must be restarted to pick up the new settings.
...
=== apippi Integrated Auth

An alternative to the out-of-the-box JMX auth is to useeapippi's own
...
To enable integrated authentication, edit `apippi-env.sh` to
uncomment these lines:

[source,bash]
----
#JVM_OPTS="$JVM_OPTS -Dapippi.jmx.remote.login.config=apippiLogin"
#JVM_OPTS="$JVM_OPTS -Djava.security.auth.login.config=$apippi_HOME/conf/apippi-jaas.config"
----
...
GRANT EXECUTE ON MBEAN 'org.apache.apippi.db:type=EndpointSnitchInfo' TO jmx;
GRANT EXECUTE ON MBEAN 'org.apache.apippi.db:type=StorageService' TO jmx;
...
[source,cql]
----
GRANT EXECUTE ON MBEAN 'org.apache.apippi.db:type=Tables,keyspace=test_keyspace,table=t1' TO ks_user;
GRANT EXECUTE ON MBEAN 'org.apache.apippi.db:type=Tables,keyspace=test_keyspace,table=*' TO ks_owner;
----
...
`apippi-env.sh` to uncomment and
