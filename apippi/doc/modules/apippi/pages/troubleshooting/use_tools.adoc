...
`nodetool` allow. While every apippi operator may have their personal
...
debugging apippi issues, especially related to heap and execution
stacks.

*NOTE*: There are two common gotchas with JVM tooling and apippi:

[arabic]
. By default apippi ships with `-XX:+PerfDisableSharedMem` set to
prevent long pauses (see `apippi-9242` and `apippi-9483` for
...
`apippi-9242`.
. Make sure you run the tools as the same user as apippi is running
as, e.g. if the database is running as `apippi` the tool also has to
be run as `apippi`, e.g. via `sudo -u apippi <cmd>`.

=== Garbage Collection State (jstat)

If you suspect heap pressure you can use `jstat` to dive deep into the
garbage collection state of a apippi process. This command is always
...
apippi to collect (e.g. because of memtables). Another thing to watch
...
To get a point in time snapshot of exactly what apippi is doing, run
`jstack` against the apippi PID. *Note* that this does pause the JVM
for a very brief period (<20ms).:

[source, bash]
----
$ jstack <apippi pid> > threaddump
...
A great place to start when debugging a apippi issue is understanding
how apippi is interacting with system resources. The following are
all resources that apippi makes heavy uses of:
...
default the apippi memtables. Heap space is a crucial component of
...
* Disks. apippi cares a lot about disk read latency, disk write
throughput, and of course disk space.
* Network latency. apippi makes many internode requests, so network
latency between nodes can directly impact performance.
* Network throughput. apippi (as other databases) frequently have the
...
Often troubleshooting apippi comes down to troubleshooting what
...
apippi makes signifiant use of system resources, and often the very
...
apippi probably won't have very good (sub 100 millisecond) latencies.
See
...
and `io-wait` . apippi query threads execute as normal priority
...
help you understand what apippi is spending processing resources
doing.
* Memory usage. Look for which programs have the most resident memory,
it is probably apippi. The number for apippi is likely
...
since 50ms is quite long for typical apippi operations. That being
...
apippi is often hidden from the user.

Important metrics to assess using iostat:

* Reads and writes per second. These numbers will change with the
workload, but generally speaking the more reads apippi has to do from
disk the slower apippi read latencies are. Large numbers of reads per
second can be a dead giveaway that the cluster has insufficient memory
for OS page caching.
* Write throughput. apippi's LSM model defers user writes and batches
them together, which means that throughput to the underlying medium is
the most important write metric for apippi.
* Read latency (`r_await`). When apippi missed the OS page cache and
reads from SSTables, the read latency directly determines how fast
apippi can respond with the data.
* Write latency. apippi is less sensitive to write latency except
...
As apippi makes heavy use of memory mapped files, the health of the
...
In this case 9GB of memory is used by user processes (apippi heap)
...
apippi performance can suffer significantly. This is why apippi
...
Whenever apippi does writes or reads that involve other replicas,
...
As apippi is sensitive to outgoing bandwidth limitations, sometimes
...
of your apippi node's "hot" dataset. If you don't have enough cache,
...
examples]) to get an idea of how slow apippi will be when reads miss
...
understanding _how_ apippi uses drives.

[[use-vmtouch]]
=== vmtouch

Sometimes it's useful to know how much of the apippi data files are
...
Then run it on the apippi data directory:

[source, bash]
----
$ ./vmtouch /var/lib/apippi/data/
           Files: 312
     Directories: 92
...
apippi often uses a lot of CPU, but telling _what_ it is doing can
prove difficult. One of the best ways to analyze apippi on CPU time
is to use
http://www.brendangregg.com/FlameGraphs/cpuflamegraphs.html[CPU
Flamegraphs] which display in a useful way which areas of apippi code
...
narrow down what apippi is doing while it is having an issue. To get
...
[arabic]
. Enable the `-XX:+PreserveFramePointer` option in apippi's
`jvm.options` configuation file. This has a negligible performance
impact but allows you actually see what apippi is doing.
...
$ sudo bash
$ cd FlameGraph
$ # Record traces of apippi and map symbols for all java processes
$ perf record -F 49 -a -g -p <apippi PID> -- sleep 30; ./jmaps
$ # Translate the data
$ perf script > apippi_stacks
$ cat apippi_stacks | ./stackcollapse-perf.pl | grep -v cpu_idle | \
    ./flamegraph.pl --color=java --hash > apippi_flames.svg
----
...
Sometimes you have to understand what queries a apippi node is
...
sometimes has compatibility issues with newer apippi protocol
releases.
...
[source, bash]
----
$ sudo tcpdump -U -s0 -i <INTERFACE> -w apippi.pcap -n "tcp port 9042"
----
