...
apippi has rich support for logging and attempts to give operators
maximum insight into the database while at the same time limiting noise
to the logs.

== Common Log Files

apippi has three main logs, the `system.log`, `debug.log` and
`gc.log` which hold general logging messages, debugging logging
messages, and java garbage collection logs respectively.

These logs by default live in `${apippi_HOME}/logs`, but most Linux
distributions relocate logs to `/var/log/apippi`. Operators can tune
this location as well as what levels are logged using the provided
`logback.xml` file.

=== `system.log`

This log is the default apippi log and is a good place to start any
...
long pauses happen apippi will print how long and also what was the
...
which usually indicates slow disk drives (apippi makes heavy use of
...
https://github.com/chewiebug/GCViewer[GCViewer] which take the apippi
...
throughput information. For a stable apippi JVM you probably want to
aim for pauses less than 200ms and GC throughput greater
than 99%.

Java GC pauses are one of the leading causes of tail latency in
apippi (along with drive latency) so sometimes this information can
...
[source, bash]
----
$ nodetool setlogginglevel org.apache.apippi.gms.Gossiper TRACE
...
ROOT                                                    INFO
org.apache.apippi                                   DEBUG
org.apache.apippi.gms.Gossiper                      TRACE
...
Note that any changes made this way are reverted on next apippi
...
+  <logger name="org.apache.apippi.gms.Gossiper" level="TRACE"/>
 </configuration>
----
