 ...
 *
 */
package org.apache.apippi.tracing;
...
public abstract class Tracing extends ExecutorLocals.Impl
{
    ...
    static
    {
        ...
        String customTracingClass = System.getProperty("apippi.custom_tracing_class");
        if (null != customTracingClass)
        {
            try
            {
                tracing = FBUtilities.construct(customTracingClass, "Tracing");
                logger.info("Using {} as tracing queries (as requested with -Dapippi.custom_tracing_class)", customTracingClass);
            }
            catch (Exception e)
            {
    ...
    public TraceState initializeFromMessage(final Message.Header header)
    {
        ...
        {
            // received a message for a session we've already closed out.  see apippi-5668
    ...
    public void traceOutgoingMessage(Message<?> message, int serializedSize, InetAddressAndPort sendTo)
    {
        try
        {
            ...
            if (state == null) // session may have already finished; see apippi-5668
            {
