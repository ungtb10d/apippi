...
import org.apache.apippi.locator.EndpointsByReplica;
...
import static org.apache.apippi.locator.Replica.fullReplica;
                                                  ...
                                                  Collection<SourceFilter> sourceFilters)
     {
         ...
         for (Replica toFetch : fetchRanges)
         {
             ...
             for (Range<Token> range : rangeAddresses.keySet())
             {
                 ...
                 //Due to apippi-5953 we can have a higher RF than we have endpoints.
    ...
    public StreamResultFuture fetchAsync()
    {
        toFetch.forEach((keyspace, sources) -> {
            ...
            sources.asMap().forEach((source, fetchReplicas) -> {
                ...
                {
                    // TODO: Also remove the files on disk. See discussion in apippi-17679
                ...
                else
                {
                    ...
                    if (remaining.size() < available.full.size() + available.trans.size())
                    {
                        ...
                        // we error out and tell them to manually reconcile it. See apippi-17679.
                        if (!RESET_BOOTSTRAP_PROGRESS.isPresent())
                        {
                            ...
                            String msg = String.format("Discovered existing bootstrap data and %s " +
                                                       ...
                                                       "and try again or set apippi.reset_bootstrap_progress=true to ignore. " +
