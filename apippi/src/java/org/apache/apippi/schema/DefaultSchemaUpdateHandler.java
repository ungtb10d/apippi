...
import org.apache.apippi.schema.SchemaTransformation.SchemaTransformationResult;
...
import org.apache.apippi.utils.concurrent.Awaitable;

import static org.apache.apippi.schema.MigrationCoordinator.MAX_OUTSTANDING_VERSION_REQUESTS;
...
public class DefaultSchemaUpdateHandler implements SchemaUpdateHandler, IEndpointStateChangeSubscriber
{
    ...
    {
        this(null, MessagingService.instance(), !apippiRelevantProperties.BOOTSTRAP_SKIP_SCHEMA_CHECK.getBoolean(), updateCallback);
    }
    ...
    public boolean waitUntilReady(Duration timeout)
    {
        ...
        logger.warn("There are nodes in the cluster with a different schema version than us, from which we did not merge schemas: " +
                    ...
                    apippiRelevantProperties.BOOTSTRAP_SKIP_SCHEMA_CHECK.getKey(),
                    apippiRelevantProperties.IGNORED_SCHEMA_CHECK_ENDPOINTS.getKey(),
                    apippiRelevantProperties.IGNORED_SCHEMA_CHECK_VERSIONS.getKey());

        if (requireSchemas)
        {
            logger.error("Didn't receive schemas for all known versions within the {}. Use -D{}=true to skip this check.",
                         timeout, apippiRelevantProperties.BOOTSTRAP_SKIP_SCHEMA_CHECK.getKey());
    ...
    public void reset(boolean local)
    {
        ...
        else
        {
            ...
            if (!migrationCoordinator.awaitSchemaRequests(apippiRelevantProperties.MIGRATION_DELAY.getLong()))
            {
