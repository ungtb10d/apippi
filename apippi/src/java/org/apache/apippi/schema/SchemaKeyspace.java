...
import org.apache.apippi.cql3.*;
import org.apache.apippi.cql3.functions.*;
...
import org.apache.apippi.service.reads.SpeculativeRetryPolicy;
import org.apache.apippi.schema.ColumnMetadata.ClusteringOrder;
import org.apache.apippi.schema.Keyspaces.KeyspacesDiff;
import org.apache.apippi.service.reads.repair.ReadRepairStrategy;
...
import static org.apache.apippi.schema.SchemaKeyspaceTables.*;
import static org.apache.apippi.utils.Simulate.With.GLOBAL_CLOCK;
...
public final class SchemaKeyspace
{
    ...
    private static final boolean FLUSH_SCHEMA_TABLES = Boolean.parseBoolean(System.getProperty("apippi.test.flush_local_schema_changes", "true"));
    private static final boolean IGNORE_CORRUPTED_SCHEMA_TABLES = Boolean.parseBoolean(System.getProperty("apippi.ignore_corrupted_schema_tables", "false"));
    ...
    {
        return KeyspaceMetadata.create(SchemaConstants.SCHEMA_KEYSPACE_NAME, KeyspaceParams.local(), org.apache.apippi.schema.Tables.of(ALL_TABLE_METADATA));
    }
    ...
    {
        // Include the serialized keyspace in case the target node missed a CREATE KEYSPACE migration (see apippi-5631).
        Mutation.SimpleBuilder builder = makeCreateKeyspaceMutation(keyspace.name, keyspace.params, timestamp);
    ...
    private static Keyspaces fetchKeyspacesWithout(Set<String> excludedKeyspaceNames)
    {
        ...
        Keyspaces.Builder keyspaces = org.apache.apippi.schema.Keyspaces.builder();
    ...
    private static Types fetchTypes(String keyspaceName)
    {
        ...
        Types.RawBuilder types = org.apache.apippi.schema.Types.rawBuilder(keyspaceName);
    ...
    private static Tables fetchTables(String keyspaceName, Types types)
    {
        ...
        Tables.Builder tables = org.apache.apippi.schema.Tables.builder();
        for (UntypedResultSet.Row row : query(query, keyspaceName))
        {
            ...
            catch (MissingColumns exc)
            {
                ...
                {
                    logger.error(errorMsg, "restart apippi with -Dapippi.ignore_corrupted_schema_tables=true and ");
                    throw exc;
                }
            }
        }
    ...
    private static DroppedColumn createDroppedColumnFromRow(UntypedResultSet.Row row)
    {
         ...
         */
        AbstractType<?> type = CQLTypeParser.parse(keyspace, row.getString("type"), org.apache.apippi.schema.Types.none());
    ...
    private static Indexes fetchIndexes(String keyspace, String table)
    {
        ...
        Indexes.Builder indexes = org.apache.apippi.schema.Indexes.builder();
    ...
    private static Triggers fetchTriggers(String keyspace, String table)
    {
        ...
        Triggers.Builder triggers = org.apache.apippi.schema.Triggers.builder();
    ...
    private static Views fetchViews(String keyspaceName, Types types)
    {
        ...
        Views.Builder views = org.apache.apippi.schema.Views.builder();
        for (UntypedResultSet.Row row : query(query, keyspaceName))
    ...
    private static Functions fetchFunctions(String keyspaceName, Types types)
    {
        ...
        return org.apache.apippi.schema.Functions.builder().add(udfs).add(udas).build();
    }
    ...
    private static UDFunction createUDFFromRow(UntypedResultSet.Row row, Types types)
    {
         ...
         */
        org.apache.apippi.cql3.functions.Function existing = Schema.instance.findFunction(name, argTypes).orElse(null);
