...
import static org.apache.apippi.streaming.StreamingChannel.Factory.Global.streamingFactory;

/**
 * apippi SSTable bulk loader.
        ...
        LifecycleTransaction.getFiles(directory.toPath(),
                                      (file, type) ->
                                      {
                                          ...
                                          if (metadata == null && // we did not find metadata
                                              directory.name().equals(Directories.BACKUPS_SUBDIR)) // and it's likely we hit apippi-16235
                                          {
                                          ...
                                          try
                                          {
                                              ...
                                              // number of keys for each endpoint. See apippi-5555 for details.
                                              ...
                                              for (Map.Entry<InetAddressAndPort, Collection<Range<Token>>> entry : ranges.entrySet())
                                              {
                                                  ...
                                                  // will generate an error on the server. See apippi-16349 for details.
                                                  if (sstableSections.isEmpty())
                                                      continue;

                                                  long estimatedKeys = sstable.estimatedKeysForRanges(tokenRanges);
                                                  Ref<SSTableReader> ref = sstable.ref();
                                                  OutgoingStream stream = new apippiOutgoingFile(StreamOperation.BULK_LOAD, ref, sstableSections, tokenRanges, estimatedKeys);
