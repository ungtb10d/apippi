...
public class FileSystemOwnershipCheck implements StartupCheck
{
    ...
    public static final String FILE_SYSTEM_CHECK_OWNERSHIP_TOKEN = "apippiOwnershipToken";
    public static final String DEFAULT_FS_OWNERSHIP_FILENAME = ".apippi_fs_ownership";
    ...
    public boolean isEnabled(StartupChecksOptions options)
    {
        ...
        return apippiRelevantProperties.FILE_SYSTEM_CHECK_ENABLE.getBoolean(enabledFromYaml);
    }

    public String getFsOwnershipFilename(Map<String, Object> config)
    {
        if (apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_FILENAME.isPresent())
        {
            logger.warn(String.format("apippi system property flag %s is deprecated and you should " +
                                      ...
                                      apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_FILENAME.getKey()));
            return apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_FILENAME.getString();
        }
        else
        {
            Object fsOwnershipFilename = config.get("ownership_filename");
            return fsOwnershipFilename == null
                   ? apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_FILENAME.getDefaultValue()
    ...
    {
        if (apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_TOKEN.isPresent())
        {
            logger.warn(String.format("apippi system property flag %s is deprecated and you should " +
                                      "use startup check configuration in apippi.yaml",
                                      apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_TOKEN.getKey()));
            return apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_TOKEN.getString();
        }
        else
        {
            Object ownershipToken = config.get("ownership_token");
            return ownershipToken == null
                   ? apippiRelevantProperties.FILE_SYSTEM_CHECK_OWNERSHIP_TOKEN.getDefaultValue()
