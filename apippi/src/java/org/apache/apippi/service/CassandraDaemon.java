...
import org.apache.apippi.db.SizeEstimatesRecorder;
...
import org.apache.apippi.db.virtual.SystemViewsKeyspace;
import org.apache.apippi.db.virtual.VirtualKeyspaceRegistry;
import org.apache.apippi.db.virtual.VirtualSchemaKeyspace;
...
import org.apache.apippi.io.sstable.SSTableHeaderFix;
...
import org.apache.apippi.metrics.DefaultNameFactory;
import org.apache.apippi.net.StartupClusterConnectivityChecker;
...
import org.apache.apippi.security.ThreadAwareSecurityManager;
...
import static org.apache.apippi.config.apippiRelevantProperties.apippi_FOREGROUND;
import static org.apache.apippi.config.apippiRelevantProperties.apippi_JMX_REMOTE_PORT;
import static org.apache.apippi.config.apippiRelevantProperties.apippi_PID_FILE;
import static org.apache.apippi.config.apippiRelevantProperties.COM_SUN_MANAGEMENT_JMXREMOTE_PORT;
import static org.apache.apippi.config.apippiRelevantProperties.JAVA_CLASS_PATH;
import static org.apache.apippi.config.apippiRelevantProperties.JAVA_VERSION;
import static org.apache.apippi.config.apippiRelevantProperties.JAVA_VM_NAME;

/**
 * The <code>apippiDaemon</code> is an abstraction for a apippi daemon
 * service, which defines not only a way to activate and deactivate it, but also
 * hooks into its lifecycle methods (see {@link #setup()}, {@link #start()},
 * {@link #stop()} and {@link #setup()}).
 */
public class apippiDaemon
{
    public static final String MBEAN_NAME = "org.apache.apippi.db:type=NativeAccess";

    private static final Logger logger;

    @VisibleForTesting
    public static apippiDaemon getInstanceForTesting()
    {
        return instance;
    }
    ...
    static {
        ...
        SharedMetricRegistries.getOrCreate("logback-metrics").addListener(new MetricRegistryListener.Base()
        {
            ...
            public void onMeterAdded(String metricName, Meter meter)
            {
                ...
                apippiMetricsRegistry.Metrics.registerMBean(meter, name);
            }
        });
        logger = LoggerFactory.getLogger(apippiDaemon.class);
    }
    ...
    private void maybeInitJmx()
    {
        ...
        // as configured in apippi-env.(sh|ps1)
        // See: apippi-11540 & apippi-11725
        if (COM_SUN_MANAGEMENT_JMXREMOTE_PORT.isPresent())
        {
            logger.warn("JMX settings in apippi-env.sh have been bypassed as the JMX connector server is " +
                        "already initialized. Please refer to apippi-env.(sh|ps1) for JMX configuration info");
            return;
        }
        ...
        // If neither is remote nor local port is set in apippi-env.(sh|ps)
        // then JMX is effectively  disabled.
        boolean localOnly = false;
        String jmxPort = apippi_JMX_REMOTE_PORT.getString();

        if (jmxPort == null)
        {
            localOnly = true;
            jmxPort = System.getProperty("apippi.jmx.local.port");
        }
    ...
    static final apippiDaemon instance = new apippiDaemon();
    ...
    public apippiDaemon(boolean runManaged)
    {
    ...
    protected void setup()
    {
        ...
        // Since apippi-14793 the local system keyspaces data are not dispatched across the data directories
        ...
        // Clean up system.size_estimates entries left lying around from missed keyspace drops (apippi-14905)
        StorageService.instance.cleanupSizeEstimates();

        // schedule periodic dumps of table size estimates into SystemKeyspace.SIZE_ESTIMATES_CF
        // set apippi.size_recorder_interval to 0 to disable
        int sizeRecorderInterval = Integer.getInteger("apippi.size_recorder_interval", 5 * 60);
        ...
        // Metrics
        String metricsReporterConfigFile = System.getProperty("apippi.metricsReporterConfigFile");
        if (metricsReporterConfigFile != null)
        {
            ...
            try
            {
                ...
                apippiMetricsRegistry.Metrics.register("jvm.buffers", new BufferPoolMetricSet(ManagementFactory.getPlatformMBeanServer()));
                apippiMetricsRegistry.Metrics.register("jvm.gc", new GarbageCollectorMetricSet());
                apippiMetricsRegistry.Metrics.register("jvm.memory", new MemoryUsageGaugeSet());
                apippiMetricsRegistry.Metrics.register("jvm.fd.usage", new FileDescriptorRatioGauge());
                // initialize metrics-reporter-config from yaml file
                URL resource = apippiDaemon.class.getClassLoader().getResource(metricsReporterConfigFile);
                if (resource == null)
                {
                ...
                else
                {
                    ...
                    ReporterConfig.loadFromFile(reportFileLocation).enableAll(apippiMetricsRegistry.Metrics);
                }
            }
            catch (Exception e)
            {
    ...
    /**
     * Initialize the apippi Daemon based on the given <a
    ...
    /**
     * Start the apippi Daemon, assuming that it has already been
    ...
    {
        String nativeFlag = System.getProperty("apippi.start_native_transport");
    ...
    public void stop()
    {
        // On linux, this doesn't entirely shut down apippi, just the RPC server.
        // jsvc takes care of taking the rest down
        logger.info("apippi shutting down...");
    ...
    public void activate()
    {
        ...
        try
        {
            ...
            String pidFile = apippi_PID_FILE.getString();
            ...
            if (apippi_FOREGROUND.getString() == null)
            {
                System.out.close();
                System.err.close();
            }
    ...
    public void startNativeTransport()
    {
        ...
        if (nativeTransportService == null)
            throw new IllegalStateException("setup() must be called first for apippiDaemon");
