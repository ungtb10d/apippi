...
import org.apache.apippi.concurrent.*;
import org.apache.apippi.config.DataStorageSpec;
import org.apache.apippi.cql3.QueryHandler;
import org.apache.apippi.dht.RangeStreamer.FetchReplica;
import org.apache.apippi.fql.FullQueryLogger;
import org.apache.apippi.fql.FullQueryLoggerOptions;
import org.apache.apippi.fql.FullQueryLoggerOptionsCompositeData;
import org.apache.apippi.io.util.File;
import org.apache.apippi.locator.ReplicaCollection.Builder.Conflict;
import org.apache.apippi.metrics.Sampler;
import org.apache.apippi.metrics.SamplingManager;
import org.apache.apippi.schema.Keyspaces;
import org.apache.apippi.service.disk.usage.DiskUsageBroadcaster;
import org.apache.apippi.service.snapshot.SnapshotLoader;
import org.apache.apippi.utils.concurrent.Future;
import org.apache.apippi.schema.TableId;
import org.apache.apippi.utils.concurrent.FutureCombiner;
import org.apache.apippi.utils.concurrent.ImmediateFuture;
import org.apache.commons.lang3.StringUtils;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import org.apache.apippi.audit.AuditLogManager;
import org.apache.apippi.audit.AuditLogOptions;
import org.apache.apippi.auth.AuthCacheService;
import org.apache.apippi.config.Config.PaxosStatePurging;
import org.apache.apippi.service.paxos.*;
import org.apache.apippi.service.paxos.cleanup.*;
import org.apache.apippi.utils.progress.ProgressListener;
import org.apache.apippi.auth.AuthKeyspace;
import org.apache.apippi.auth.AuthSchemaChangeListener;
...
import org.apache.apippi.config.DurationSpec;
...
import org.apache.apippi.db.virtual.VirtualKeyspaceRegistry;
import org.apache.apippi.dht.*;
import org.apache.apippi.dht.Range;
import org.apache.apippi.dht.Token.TokenFactory;
import org.apache.apippi.exceptions.*;
import org.apache.apippi.gms.*;
import org.apache.apippi.hints.HintsService;
import org.apache.apippi.io.sstable.SSTableLoader;
import org.apache.apippi.io.sstable.format.SSTableFormat;
import org.apache.apippi.io.sstable.format.VersionAndType;
import org.apache.apippi.io.util.FileUtils;
import org.apache.apippi.io.util.PathUtils;
import org.apache.apippi.locator.*;
import org.apache.apippi.metrics.StorageMetrics;
import org.apache.apippi.net.*;
import org.apache.apippi.repair.*;
import org.apache.apippi.repair.messages.RepairOption;
import org.apache.apippi.schema.CompactionParams.TombstoneOption;
import org.apache.apippi.schema.KeyspaceMetadata;
import org.apache.apippi.schema.ReplicationParams;
import org.apache.apippi.schema.Schema;
import org.apache.apippi.schema.SchemaConstants;
import org.apache.apippi.schema.SchemaTransformations;
import org.apache.apippi.schema.SystemDistributedKeyspace;
import org.apache.apippi.schema.TableMetadata;
import org.apache.apippi.schema.TableMetadataRef;
import org.apache.apippi.schema.ViewMetadata;
import org.apache.apippi.service.snapshot.SnapshotManager;
import org.apache.apippi.service.snapshot.TableSnapshot;
import org.apache.apippi.net.AsyncOneResponse;
import org.apache.apippi.net.MessagingService;
import org.apache.apippi.streaming.*;
import org.apache.apippi.tracing.TraceKeyspace;
import org.apache.apippi.transport.ClientResourceLimits;
import org.apache.apippi.transport.ProtocolVersion;
import org.apache.apippi.utils.*;
...
import org.apache.apippi.utils.progress.ProgressEvent;
...
import org.apache.apippi.utils.progress.jmx.JMXProgressSupport;
...
import static org.apache.apippi.config.apippiRelevantProperties.BOOTSTRAP_SCHEMA_DELAY_MS;
import static org.apache.apippi.config.apippiRelevantProperties.BOOTSTRAP_SKIP_SCHEMA_CHECK;
import static org.apache.apippi.config.apippiRelevantProperties.DRAIN_EXECUTOR_TIMEOUT_MS;
import static org.apache.apippi.config.apippiRelevantProperties.REPLACEMENT_ALLOW_EMPTY;
import static org.apache.apippi.index.SecondaryIndexManager.getIndexName;
import static org.apache.apippi.index.SecondaryIndexManager.isIndexColumnFamily;
import static org.apache.apippi.net.NoPayload.noPayload;
import static org.apache.apippi.net.Verb.REPLICATION_DONE_REQ;
import static org.apache.apippi.service.ActiveRepairService.*;
import static org.apache.apippi.utils.Clock.Global.currentTimeMillis;
import static org.apache.apippi.utils.Clock.Global.nanoTime;
import static org.apache.apippi.utils.FBUtilities.now;
...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    {
        String newdelay = apippiRelevantProperties.RING_DELAY.getString();
        if (newdelay != null)
        {
    ...
    public static final boolean useStrictConsistency = Boolean.parseBoolean(System.getProperty("apippi.consistent.rangemovement", "true"));
    private static final boolean allowSimultaneousMoves = Boolean.parseBoolean(System.getProperty("apippi.consistent.simultaneousmoves.allow","false"));
    private static final boolean joinRing = Boolean.parseBoolean(System.getProperty("apippi.join_ring", "true"));
    ...
    public StorageService()
    {
        ...
        jmxObjectName = "org.apache.apippi.db:type=StorageService";
    ...
    public void registerDaemon(apippiDaemon daemon)
    {
        this.daemon = daemon;
    }
    ...
    private synchronized UUID prepareForReplacement() throws ConfigurationException
    {
        ...
        if (!shouldBootstrap() && !Boolean.getBoolean("apippi.allow_unsafe_replace"))
            throw new RuntimeException("Replacing a node without bootstrapping risks invalidating consistency " +
                                       "guarantees as the expected data may not be present until repair is run. " +
                                       "To perform this operation, please restart with " +
                                       "-Dapippi.allow_unsafe_replace=true");
    ...
    {
        if (Boolean.getBoolean("apippi.allow_unsafe_join"))
        {
            logger.warn("Skipping endpoint collision check as apippi.allow_unsafe_join=true");
            return;
        }
        ...
        if (!Gossiper.instance.isSafeForStartup(FBUtilities.getBroadcastAddressAndPort(), localHostId, shouldBootstrap(), epStates))
        {
            throw new RuntimeException(String.format("A node with address %s already exists, cancelling join. " +
                                                     "Use apippi.replace_address if you want to replace this node.",
        ...
        if (shouldBootstrap() && useStrictConsistency && !allowSimultaneousMoves())
        {
            for (Map.Entry<InetAddressAndPort, EndpointState> entry : epStates.entrySet())
            {
                ...
                if (state.equals(VersionedValue.STATUS_BOOTSTRAPPING) || state.equals(VersionedValue.STATUS_LEAVING) || state.equals(VersionedValue.STATUS_MOVING))
                    throw new UnsupportedOperationException("Other bootstrapping/leaving/moving nodes detected, cannot bootstrap while apippi.consistent.rangemovement is true");
            }
        }
    }
    ...
    {
        logger.info("apippi version: {}", FBUtilities.getReleaseVersionString());
        ...
        try
        {
            // Ensure StorageProxy is initialized on start-up; see apippi-3797.
            Class.forName("org.apache.apippi.service.StorageProxy");
            // also IndexSummaryManager, which is otherwise unreferenced
            Class.forName("org.apache.apippi.io.sstable.IndexSummaryManager");
        }
        ...
        if (Boolean.parseBoolean(System.getProperty("apippi.load_ring_state", "true")))
        {
        ...
        if (!Boolean.parseBoolean(System.getProperty("apippi.start_gossip", "true")))
        {
    ...
    public boolean isReplacing()
    {
        ...
        if (System.getProperty("apippi.replace_address_first_boot", null) != null && SystemKeyspace.bootstrapComplete())
        {
    ...
    private void prepareToJoin() throws ConfigurationException
    {
        if (!joined)
        {
            ...
            {
                if (Boolean.getBoolean("apippi.override_decommission"))
                {
                    logger.warn("This node was decommissioned, but overriding by operator request.");
                    SystemKeyspace.setBootstrapState(SystemKeyspace.BootstrapState.COMPLETED);
                }
                else
                    throw new ConfigurationException("This node was decommissioned and will not rejoin the ring unless apippi.override_decommission=true has been set, or all existing data is removed and the node is bootstrapped again");
            }

            if (DatabaseDescriptor.getReplaceTokens().size() > 0 || DatabaseDescriptor.getReplaceNode() != null)
                throw new RuntimeException("Replace method removed; use apippi.replace_address instead");
            ...
            if (replacing)
            {
                ...
                {
                    //only go into hibernate state if replacing the same address (apippi-8523)
                              ...
                              long ringTimeoutMillis) throws ConfigurationException
    {
        ...
        // to get schema info from gossip which defeats the purpose.  See apippi-4427 for the gory details.
     ...
     * See org.apache.apippi.distributed.impl.Instance#startup(org.apache.apippi.distributed.api.ICluster)
     */
    @VisibleForTesting
    ...
    public Collection<InetAddressAndPort> prepareForBootstrap(long schemaTimeoutMillis, long ringTimeoutMillis)
    {
            ...
            ))
        {
            ...
            throw new UnsupportedOperationException(String.format("Other bootstrapping/leaving/moving nodes detected, cannot bootstrap while apippi.consistent.rangemovement is true. Nodes detected, bootstrapping: %s; leaving: %s; moving: %s;", bootstrapTokens, leavingTokens, movingTokens));
        }
    ...
    public boolean bootstrap(final Collection<Token> tokens, long bootstrapTimeoutMillis)
    {
        ...
        if (Boolean.getBoolean("apippi.reset_bootstrap_progress"))
        {
    ...
    */
    ...
    private void updateTokenMetadata(InetAddressAndPort endpoint, Iterable<Token> tokens, Set<InetAddressAndPort> endpointsToRemove)
    {
        ...
        for (InetAddressAndPort ep : endpointsToRemove)
        {
            ...
            if (replacing && ep.equals(DatabaseDescriptor.getReplaceAddress()))
                Gossiper.instance.replacementQuarantine(ep); // quarantine locally longer than normally; see apippi-8260
        }
    ...
    private void handleStateNormal(final InetAddressAndPort endpoint, final String status)
    {
        ...
        // Order Matters, TM.updateHostID() should be called before TM.updateNormalToken(), (see apippi-4300).
    ...
    public void onRestart(InetAddressAndPort endpoint, EndpointState state)
    {
        ...
        // want to update that before we mark the node live again to avoid problems like apippi-11128.
    ...
    public void takeSnapshot(String tag, Map<String, String> options, String... entities) throws IOException
    {
        ...
        if (ttl != null)
        {
            int minAllowedTtlSecs = apippiRelevantProperties.SNAPSHOT_MIN_ALLOWED_TTL_SECONDS.getInt();
    ...
    private void repairPaxosForTopologyChange(String reason)
    {
        ...
        int retries = 0;
        int maxRetries = Integer.getInteger("apippi.paxos_repair_on_topology_change_retries", 10);
        int delaySec = Integer.getInteger("apippi.paxos_repair_on_topology_change_retry_delay_seconds", 10);
    ...
    /*
     * In apippi-17668, JMX setters that did not throw standard exceptions were deprecated in favor of ones that do.
    ...
    */
    ...
    private List<Pair<Range<Token>, Long>> getSplits(List<Token> tokens, int splitCount, ColumnFamilyStore cfs)
    {
        ...
        for (int i = 1; i <= splitCount; i++)
        {
            ...
            // always return an estimate > 0 (see apippi-7322)
    ...
    protected synchronized void drain(boolean isFinalShutdown) throws IOException, InterruptedException, ExecutionException
    {
        ...
        try
        {
            ...
            // like apippi-5151. Don't bother with progress tracking since system data is tiny.
     ...
     * out attempting to use apippi calls which will no longer work.
     * @param hook: the code to run
     * @return true on success, false if apippi is already shutting down, in which case the runnable
     * has NOT been added.
     */
     ...
     * running in the same JVM that apippi needs to work and should shut down later.
     * @param hook: the code to run
     * @return true on success, false if apippi is already shutting down, in which case the runnable has NOT been
     * added.
     */
     ...
     * #{@inheritDoc}
     */
    public List<String> sampleKeyRange() // do not rename to getter - see apippi-4452 for details
    {
        List<DecoratedKey> keys = new ArrayList<>();
    ...
    public void enableFullQueryLogger(String path, String rollCycle, Boolean blocking, int maxQueueWeight, long maxLogSize, String archiveCommand, int maxArchiveRetries)
    {
        ...
        Preconditions.checkNotNull(path, "apippi.yaml did not set log_dir and not set as parameter");
