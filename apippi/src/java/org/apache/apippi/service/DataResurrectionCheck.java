...
import static org.apache.apippi.exceptions.StartupException.ERR_WRONG_DISK_STATE;
import static org.apache.apippi.exceptions.StartupException.ERR_WRONG_MACHINE_STATE;
...
public class DataResurrectionCheck implements StartupCheck
{
    ...
    public static final String DEFAULT_HEARTBEAT_FILE = "apippi-heartbeat";
    ...
    public void execute(StartupChecksOptions options) throws StartupException
    {
        ...
        if (!violations.isEmpty())
        {
            ...
            String exceptionMessage = format("There are tables for which gc_grace_seconds is older " +
                                             "than the lastly known time apippi node was up based " +
                                             "on its heartbeat %s with timestamp %s. apippi node will not start " +
    ...
    public void postAction(StartupChecksOptions options)
    {
        ...
        if (options.isEnabled(StartupChecks.StartupCheckType.check_data_resurrection))
        {
            ...
            }, 0, apippiRelevantProperties.CHECK_DATA_RESURRECTION_HEARTBEAT_PERIOD.getInt(), MILLISECONDS);
        }
    }
