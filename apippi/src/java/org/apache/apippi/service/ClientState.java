...
import org.apache.apippi.dht.Datacenters;
...
public class ClientState
{
    ...
    static
    {
        ...
        String customHandlerClass = System.getProperty("apippi.custom_query_handler_class");
        if (customHandlerClass != null)
        {
            try
            {
                handler = FBUtilities.construct(customHandlerClass, "QueryHandler");
                logger.info("Using {} as query handler for native protocol queries (as requested with -Dapippi.custom_query_handler_class)", customHandlerClass);
            }
            catch (Exception e)
            {
