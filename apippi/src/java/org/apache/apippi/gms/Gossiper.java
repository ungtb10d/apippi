...
import org.apache.apippi.utils.ExpiringMemoizingSupplier;
...
import org.apache.apippi.utils.RecomputingSupplier;
import org.apache.apippi.utils.concurrent.UncheckedInterruptedException;
import org.apache.apippi.utils.concurrent.NotScheduledFuture;

import static org.apache.apippi.concurrent.ExecutorFactory.Global.executorFactory;
import static org.apache.apippi.config.apippiRelevantProperties.DISABLE_GOSSIP_ENDPOINT_REMOVAL;
import static org.apache.apippi.config.apippiRelevantProperties.GOSSIPER_QUARANTINE_DELAY;
import static org.apache.apippi.config.apippiRelevantProperties.GOSSIPER_SKIP_WAITING_TO_SETTLE;
import static org.apache.apippi.config.apippiRelevantProperties.SHUTDOWN_ANNOUNCE_DELAY_IN_MS;
import static org.apache.apippi.config.DatabaseDescriptor.getClusterName;
import static org.apache.apippi.config.DatabaseDescriptor.getPartitionerName;
import static org.apache.apippi.net.NoPayload.noPayload;
import static org.apache.apippi.net.Verb.ECHO_REQ;
import static org.apache.apippi.net.Verb.GOSSIP_DIGEST_SYN;
...
import static org.apache.apippi.gms.VersionedValue.BOOTSTRAPPING_STATUS;
...
{
    public static final String MBEAN_NAME = "org.apache.apippi.net:type=Gossiper";

    public static class Props
    {
        public static final String DISABLE_THREAD_VALIDATION = "apippi.gossip.disable_thread_validation";
    }
    ...
    // Timestamp to prevent processing any in-flight messages for we've not send any SYN yet, see apippi-12653.
    ...
    private static final ExpiringMemoizingSupplier.Memoized<apippiVersion> NO_UPGRADE_IN_PROGRESS = new ExpiringMemoizingSupplier.Memoized<>(null);
    private static final ExpiringMemoizingSupplier.NotMemoized<apippiVersion> CURRENT_NODE_VERSION = new ExpiringMemoizingSupplier.NotMemoized<>(SystemKeyspace.CURRENT_VERSION);
    final Supplier<ExpiringMemoizingSupplier.ReturnValue<apippiVersion>> upgradeFromVersionSupplier = () ->
    {
        ...
        apippiVersion minVersion = SystemKeyspace.CURRENT_VERSION;
        ...
        for (InetAddressAndPort host : endpointStateMap.keySet())
        {
            ...
            apippiVersion version = getReleaseVersion(host);
    ...
    private final Supplier<apippiVersion> upgradeFromVersionMemoized = ExpiringMemoizingSupplier.memoizeWithExpiration(upgradeFromVersionSupplier, 1, TimeUnit.MINUTES);

    @VisibleForTesting
    public void expireUpgradeFromVersion()
    {
        upgradeInProgressPossible = true;
        ((ExpiringMemoizingSupplier<apippiVersion>) upgradeFromVersionMemoized).expire();
    }
    ...
    {
        String newVLT =  System.getProperty("apippi.very_long_time_ms");
        if (newVLT != null)
        {
            ...
            try
            {
                ...
                if (gDigests.size() > 0)
                {
                    ...
                    /* Gossip to a seed if we did not do so above, or we have seen less nodes
                       ...
                       See apippi-150 for more exposition. */
    ...
    private final RecomputingSupplier<apippiVersion> minVersionSupplier = new RecomputingSupplier<>(this::computeMinVersion, executor);
     ...
     * in the ring but have no state in gossip (see apippi-16213).
     *
     ...
     *
     * When a {@link EndpointState} is "empty" then it is filtered out and not added to the delta state (see apippi-16213).
     */
    ...
    public synchronized Map<InetAddressAndPort, EndpointState> doShadowRound(Set<InetAddressAndPort> peers)
    {
        ...
        try
        {
            ...
            {
                if (slept % 5000 == 0)
                { // apippi-8072, retry at the beginning and every 5 seconds
                ...
                {
                    // if we got here no peers could be gossiped to. If we're a seed that's OK, but otherwise we stop. See apippi-13851
                    if (!isSeed)
    ...
    @Nullable
    public apippiVersion getReleaseVersion(InetAddressAndPort ep)
    {
    ...
    public static void waitToSettle()
    {
        ...
        while (numOkay < GOSSIP_SETTLE_POLL_SUCCESSES_REQUIRED)
        {
            ...
            {
                logger.warn("Gossip not settled but startup forced by apippi.skip_wait_for_gossip_to_settle. Gossip total polls: {}",
                            totalPolls);
                break;
            }
        }
    ...
    {
        return isUpgradingFromVersionLowerThan(apippiVersion.apippi_4_0) || // this is quite obvious
     ...
     */
    public boolean isUpgradingFromVersionLowerThan(apippiVersion referenceVersion)
    {
    ...
    @VisibleForTesting
    public Pair<Boolean, apippiVersion> isUpgradingFromVersionLowerThanC17653(apippiVersion referenceVersion)
    {
        apippiVersion v = upgradeFromVersionMemoized.get();
        if (apippiVersion.NULL_VERSION.equals(v) && scheduledGossipTask == null)
    ...
    @Nullable
    public apippiVersion getMinVersion(long delay, TimeUnit timeUnit)
    {
        try
        {
    ...
    private apippiVersion computeMinVersion()
    {
        apippiVersion minVersion = null;
                                                        ...
                                                        Gossiper.instance.getUnreachableMembers()))
        {
            ...
            try
            {
                version = new apippiVersion(versionString);
            }
            catch (Throwable t)
            {
