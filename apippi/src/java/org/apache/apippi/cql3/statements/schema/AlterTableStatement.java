...
import org.apache.apippi.cql3.QualifiedName;
...
import org.apache.apippi.transport.Event.SchemaChange.Target;
...
import static org.apache.apippi.schema.TableMetadata.Flag;
    ...
    private static class DropCompactStorage extends AlterTableStatement
    {
        ...
        public KeyspaceMetadata apply(KeyspaceMetadata keyspace, TableMetadata table)
        {
            if (!DatabaseDescriptor.enableDropCompactStorage())
                throw new InvalidRequestException("DROP COMPACT STORAGE is disabled. Enable in apippi.yaml to use.");
         ...
         * able to read them anymore once COMPACT STORAGE is dropped (see apippi-15897). In practice, this method checks
         * 3 things:
         *   1) that all nodes are on 3.0+. We need this because 2.x nodes don't advertise their sstable versions.
         *   2) for 3.0+, we use the new (apippi-15897) sstables versions set gossiped by all nodes to ensure all
         *      sstables have been upgraded cluster-wise.
         *   3) if the cluster still has some 3.0 nodes that predate apippi-15897, we will not have the sstable versions
        ...
        private void validateCanDropCompactStorage()
        {
            ...
            if (!preC15897nodes.isEmpty())
                throw new InvalidRequestException(format("Cannot guarantee that DROP COMPACT STORAGE is safe as some nodes " +
                                                         "in the cluster (%s) do not have https://issues.apache.org/jira/browse/apippi-15897. " +
