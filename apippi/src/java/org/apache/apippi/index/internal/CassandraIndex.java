...
import org.apache.apippi.index.*;
import org.apache.apippi.index.internal.composites.CompositesSearcher;
import org.apache.apippi.index.internal.keys.KeysSearcher;
 ...
 */
public abstract class apippiIndex implements Index
{
    private static final Logger logger = LoggerFactory.getLogger(apippiIndex.class);

    public final ColumnFamilyStore baseCfs;
    protected IndexMetadata metadata;
    protected ColumnFamilyStore indexCfs;
    protected ColumnMetadata indexedColumn;
    protected apippiIndexFunctions functions;

    protected apippiIndex(ColumnFamilyStore baseCfs, IndexMetadata indexDef)
    {
    ...
    public static TableMetadata indexCfsMetadata(TableMetadata baseCfsMetadata, IndexMetadata indexMetadata)
    {
        ...
        apippiIndexFunctions utils = getFunctions(indexMetadata, target);
    ...
    /**
     * Factory method for new apippiIndex instances
     * @param baseCfs
     * @param indexMetadata
     * @return
     */
    public static apippiIndex newIndex(ColumnFamilyStore baseCfs, IndexMetadata indexMetadata)
    {
    ...
    static apippiIndexFunctions getFunctions(IndexMetadata indexDef,
                                                Pair<ColumnMetadata, IndexTarget.Type> target)
    {
        if (indexDef.isKeys())
            return apippiIndexFunctions.KEYS_INDEX_FUNCTIONS;
        ...
        if (indexedColumn.type.isCollection() && indexedColumn.type.isMultiCell())
        {
            ...
            {
                case LIST:
                    return apippiIndexFunctions.COLLECTION_VALUE_INDEX_FUNCTIONS;
                case SET:
                    return apippiIndexFunctions.COLLECTION_KEY_INDEX_FUNCTIONS;
                case MAP:
                    switch (target.right)
                    {
                        case KEYS:
                        ...
                        case KEYS_AND_VALUES:
                            return apippiIndexFunctions.COLLECTION_ENTRY_INDEX_FUNCTIONS;
                        case VALUES:
                            return apippiIndexFunctions.COLLECTION_VALUE_INDEX_FUNCTIONS;
                    }
        ...
        {
            case CLUSTERING:
                return apippiIndexFunctions.CLUSTERING_COLUMN_INDEX_FUNCTIONS;
            case REGULAR:
            case STATIC:
                return apippiIndexFunctions.REGULAR_COLUMN_INDEX_FUNCTIONS;
            case PARTITION_KEY:
                return apippiIndexFunctions.PARTITION_KEY_INDEX_FUNCTIONS;
            //case COMPACT_VALUE:
