...
import org.apache.apippi.net.OutboundConnectionSettings.Framing;
...
import static org.apache.apippi.auth.IInternodeAuthenticator.InternodeConnectionDirection.INBOUND;
...
import static org.apache.apippi.net.MessagingService.*;
import static org.apache.apippi.net.SocketFactory.WIRETRACE;
import static org.apache.apippi.net.SocketFactory.newSslHandler;
        ...
        if (!channelFuture.awaitUninterruptibly().isSuccess())
        {
            ...
            if (causeString.contains("in use"))
            {
                throw new ConfigurationException(bind + " is in use by another process.  Change listen_address:storage_port " +
                                                 "in apippi.yaml to values that do not conflict with other services");
            }
            else if (causeString.contains("cannot assign requested address"))
            {
                throw new ConfigurationException("Unable to bind to address " + bind
                                                 + ". Set listen_address in apippi.yaml to an interface you can bind to, e.g., your private IP address on EC2");
            }
            else
            {
