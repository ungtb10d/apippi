...
import org.apache.apippi.utils.OverlapIterator;
import org.apache.apippi.utils.concurrent.Refs;

import static org.apache.apippi.db.lifecycle.SSTableIntervalTree.buildIntervals;
...
public class CompactionController extends AbstractCompactionController
{
    ...
    public CompactionController(ColumnFamilyStore cfs, Set<SSTableReader> compacting, int gcBefore, RateLimiter limiter, TombstoneOption tombstoneOption)
    {
        ...
        if (NEVER_PURGE_TOMBSTONES)
            logger.warn("You are running with -Dapippi.never_purge_tombstones=true, this is dangerous!");
    }
     ...
     * This strategy can retain for a long time a lot of sstables on disk (see apippi-13418) so this option
