...
public class apippiKeyspaceWriteHandler implements KeyspaceWriteHandler
{
    private final Keyspace keyspace;

    public apippiKeyspaceWriteHandler(Keyspace keyspace)
    {
        this.keyspace = keyspace;
    }

    @Override
    @SuppressWarnings("resource") // group is closed when apippiWriteContext is closed
    public WriteContext beginWrite(Mutation mutation, boolean makeDurable) throws RequestExecutionException
    {
        ...
        try
        {
            ...
            return new apippiWriteContext(group, position);
        }
        catch (Throwable t)
        {
            if (group != null)
    ...
    private WriteContext createEmptyContext()
    {
        ...
        {
            group = Keyspace.writeOrder.start();
            return new apippiWriteContext(group, null);
        }
        catch (Throwable t)
        {
            if (group != null)
