...
import org.apache.apippi.db.rows.ThrottledUnfilteredIterator;
...
import org.apache.apippi.streaming.IncomingStream;
import org.apache.apippi.streaming.StreamReceiver;
...
public class apippiStreamReceiver implements StreamReceiver
{
    private static final Logger logger = LoggerFactory.getLogger(apippiStreamReceiver.class);

    private static final int MAX_ROWS_PER_BATCH = Integer.getInteger("apippi.repair.mutation_repair_rows_per_batch", 100);
    ...
    public apippiStreamReceiver(ColumnFamilyStore cfs, StreamSession session, int totalFiles)
    {
        this.cfs = cfs;
    ...
    public static apippiStreamReceiver fromReceiver(StreamReceiver receiver)
    {
        Preconditions.checkArgument(receiver instanceof apippiStreamReceiver);
        return (apippiStreamReceiver) receiver;
    }

    private static apippiIncomingFile getFile(IncomingStream stream)
    {
        Preconditions.checkArgument(stream instanceof apippiIncomingFile, "Wrong stream type: {}", stream);
        return (apippiIncomingFile) stream;
    }
    ...
    {
        apippiIncomingFile file = getFile(stream);
    ...
    public synchronized LifecycleNewTracker createLifecycleNewTracker()
    {
        return new LifecycleNewTracker()
        {
            ...
            {
                synchronized (apippiStreamReceiver.this)
                {
                    txn.trackNew(table);
                }
            }
    ...
    public void finished()
    {
        ...
        try (Refs<SSTableReader> refs = Refs.ref(readers))
        {
            ...
            else
            {
                ...
                // add sstables (this will build secondary indexes too, see apippi-10130)
