 ...
 */
public class apippiIncomingFile implements IncomingStream
{
    ...
    private static final Logger logger = LoggerFactory.getLogger(apippiIncomingFile.class);

    public apippiIncomingFile(ColumnFamilyStore cfs, StreamSession session, StreamMessageHeader header)
    {
        this.cfs = cfs;
    ...
    {
        apippiStreamHeader streamHeader = apippiStreamHeader.serializer.deserialize(in, version);
        ...
        {
            reader = new apippiEntireSSTableStreamReader(header, streamHeader, session);
            numFiles = streamHeader.componentManifest.components().size();
        }
        else if (streamHeader.isCompressed())
            reader = new apippiCompressedStreamReader(header, streamHeader, session);
        else
            reader = new apippiStreamReader(header, streamHeader, session);
    ...
    public boolean equals(Object o)
    {
        ...
        apippiIncomingFile that = (apippiIncomingFile) o;
