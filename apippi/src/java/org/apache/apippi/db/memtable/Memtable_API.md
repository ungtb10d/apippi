...
[CEP-11](https://cwiki.apache.org/confluence/display/apippi/CEP-11%3A+Pluggable+memtable+implementations) 
/ [apippi-17034](https://issues.apache.org/jira/browse/apippi-17034)
...
The memtable configurations are defined in `apippi.yaml`, using the following format:
...
apippi currently comes with three memtable implementations:

- `SkipListMemtable` is the default and matches the memtable format of apippi versions up to 4.1. It organizes
...
do not have a definition for that configuration, cannot instantiate the class, or are still on a version of apippi
...
The `MemtableParams` class will look for the specified class name (prefixed with `org.apache.apippi.db.memtable.`
...
`memtable_allocation_type` in `apippi.yaml` and `memtable_flush_period_in_ms` in the table schema.
