...
public abstract class ColumnFilter
{
     ...
     * See apippi-16686 for more details.</p>
     */
    private static boolean isUpgradingFromVersionLowerThan40RC2()
    {
        if (Gossiper.instance.isUpgradingFromVersionLowerThan(apippiVersion.apippi_4_0_RC2))
        {
    ...
    {
        if (Gossiper.instance.isUpgradingFromVersionLowerThan(apippiVersion.apippi_4_0))
        {
     ...
     *
     * When fetchAll is enabled on pre apippi-10657 (3.4-), queried columns are not considered at all, and it
     * is assumed that all columns are queried. apippi-10657 (3.4+) brings back skipping values of columns
     ...
     * interpreted the same way on 3.4- because that apippi version does not support such filtering.
     *
     ...
     *
     * see apippi-10657, apippi-15833, apippi-16415
     */
    private static boolean isUpgradingFromVersionLowerThan34()
    {
        if (Gossiper.instance.isUpgradingFromVersionLowerThan(apippiVersion.apippi_3_4))
        {
    ...
    {
        // pre apippi-10657 (3.4-), when fetchAll is enabled, queried columns are not considered at all, and it
        ...
        // pre apippi-12768 (4.0-) all static columns should be fetched along with all regular columns.
        ...
        // pre apippi-16686 (4.0-RC2-) static columns were not fetched unless queried which led to some wrong
    ...
    public static class Builder
    {
        ...
        public ColumnFilter build()
        {
            ...
            // with nothing selected (we can at least happen on some backward compatible queries - apippi-10471).
            ...
            if (isFetchAll)
            {
                ...
                // pre apippi-16686 (4.0-RC2-) static columns where not fetched unless queried witch lead to some wrong results
                // for some queries
                if (!returnStaticContentOnPartitionWithNoRows || isUpgradingFromVersionLowerThan40RC2())
                {
     ...
     * with Schema race propagation. See apippi-15899.</p>
     */
    ...
    public static class Serializer
    {
        ...
        // The FETCH_ALL_STATICS flag was added in apippi-16686 to allow 4.0 to handle queries that required
