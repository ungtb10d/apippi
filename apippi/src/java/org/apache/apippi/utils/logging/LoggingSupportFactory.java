...
public class LoggingSupportFactory
{
    ...
    public static LoggingSupport getLoggingSupport()
    {
        if (loggingSupport == null)
        {
            ...
            {
                loggingSupport = FBUtilities.instanceOrConstruct("org.apache.apippi.utils.logging.LogbackLoggingSupport", "LogbackLoggingSupport");
            }
            else
            {
                loggingSupport = new NoOpFallbackLoggingSupport();
                logger.warn("You are using apippi with an unsupported deployment. The intended logging implementation library logback is not used by slf4j. Detected slf4j logger factory: {}. "
