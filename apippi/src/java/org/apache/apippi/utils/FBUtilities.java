...
import org.apache.apippi.utils.concurrent.*;
...
import org.apache.apippi.audit.IAuditLogger;
import org.apache.apippi.auth.AllowAllNetworkAuthorizer;
...
import org.apache.apippi.io.sstable.metadata.MetadataComponent;
import org.apache.apippi.io.sstable.metadata.MetadataType;
import org.apache.apippi.io.sstable.metadata.ValidationMetadata;
...
import org.apache.apippi.security.ISslContextFactory;
import org.apache.apippi.utils.concurrent.UncheckedInterruptedException;
import org.objectweb.asm.Opcodes;

import static org.apache.apippi.config.apippiRelevantProperties.LINE_SEPARATOR;
import static org.apache.apippi.config.apippiRelevantProperties.USER_HOME;
import static org.apache.apippi.io.util.File.WriteMode.OVERWRITE;
...
public class FBUtilities
{
    ...
    private static int availableProcessors = Integer.getInteger("apippi.available_processors", DatabaseDescriptor.getAvailableProcessors());
            ...
            if (DatabaseDescriptor.getListenAddress() == null)
            {
                try
                {
                    ...
                    logger.info("InetAddress.getLocalHost() was used to resolve listen_address to {}, double check this is "
                                + "correct. Please check your node's config and set the listen_address in apippi.yaml accordingly if applicable.",
                                localInetAddress);
                }
                catch(UnknownHostException e)
                {
                    logger.info("InetAddress.getLocalHost() could not resolve the address for the hostname ({}), please "
                                + "check your node's config and set the listen_address in apippi.yaml. Falling back to {}",
                                e,
                                InetAddress.getLoopbackAddress());
                    // apippi-15901 fallback for misconfigured nodes
    ...
    public static File apippiTriggerDir()
    {
        File triggerDir = null;
        if (System.getProperty("apippi.triggers_dir") != null)
        {
            triggerDir = new File(System.getProperty("apippi.triggers_dir"));
        }
        else
        {
    ...
    {
        try (InputStream in = FBUtilities.class.getClassLoader().getResourceAsStream("org/blcksec/apippi/config/version.properties"))
        {
            if (in == null)
            {
                return null;
    ...
    public static String getReleaseVersionString()
    {
        ...
        if (props == null)
            return System.getProperty("apippi.releaseVersion", UNKNOWN_RELEASE_VERSION);
        return props.getProperty("apippiVersion");
    }
    ...
    public static String getGitSHA()
    {
        Properties props = getVersionProperties();
        if (props == null)
            return System.getProperty("apippi.gitSHA", UNKNOWN_GIT_SHA);
     ...
     */
    public static <T> org.apache.apippi.utils.concurrent.Future<List<T>> allOf(Collection<? extends org.apache.apippi.utils.concurrent.Future<? extends T>> futures)
    {
    ...
    static IPartitioner newPartitioner(String partitionerClassName, Optional<AbstractType<?>> comparator) throws ConfigurationException
    {
        if (!partitionerClassName.contains("."))
            partitionerClassName = "org.apache.apippi.dht." + partitionerClassName;

        if (partitionerClassName.equals("org.apache.apippi.dht.LocalPartitioner"))
        {
    ...
    public static IAuthorizer newAuthorizer(String className) throws ConfigurationException
    {
        if (!className.contains("."))
            className = "org.apache.apippi.auth." + className;
    ...
    {
        if (!className.contains("."))
            className = "org.apache.apippi.audit." + className;
    ...
    {
        if (!className.contains("."))
            className = "org.apache.apippi.security." + className;
    ...
    {
        File historyDir = new File(USER_HOME.getString(), ".apippi");
