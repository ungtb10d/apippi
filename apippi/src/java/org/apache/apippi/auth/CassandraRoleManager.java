...
import static org.apache.apippi.service.QueryState.forInternalCalls;
 ...
 *
 * Additionally, if org.apache.apippi.auth.PasswordAuthenticator is used,
 ...
 * apippiRoleManager for those functions.
 *
 * Alternative IAuthenticator implementations may be used in conjunction with
 * apippiRoleManager, but WITH PASSWORD = 'password' will not be supported
 * in CREATE/ALTER ROLE statements.
 *
 * Such a configuration could be implemented using a custom IRoleManager that
 * extends apippiRoleManager and which includes Option.PASSWORD in the {@code Set<Option>}
 ...
 */
public class apippiRoleManager implements IRoleManager
{
    private static final Logger logger = LoggerFactory.getLogger(apippiRoleManager.class);

    public static final String DEFAULT_SUPERUSER_NAME = "apippi";
    static final String DEFAULT_SUPERUSER_PASSWORD = "apippi";
    ...
    private static void setupDefaultRole()
    {
        if (StorageService.instance.getTokenMetadata().sortedTokens().isEmpty())
            throw new IllegalStateException("apippiRoleManager skipped default role setup: no known tokens in ring");
        ...
        {
            logger.warn("apippiRoleManager skipped default role setup: some nodes were not ready");
            throw e;
        }
    }
    ...
    {
        // Try looking up the 'apippi' default role first, to avoid the range query if possible.
    ...
    protected static ConsistencyLevel consistencyForRoleWrite(String role)
    {
        return role.equals(DEFAULT_SUPERUSER_NAME) ?
               ...
               apippiAuthorizer.authWriteConsistencyLevel();
    }
    ...
    protected static ConsistencyLevel consistencyForRoleRead(String role)
    {
        return role.equals(DEFAULT_SUPERUSER_NAME) ?
               DEFAULT_SUPERUSER_CONSISTENCY_LEVEL :
               apippiAuthorizer.authReadConsistencyLevel();
    }
    ...
    public Supplier<Map<RoleResource, Set<Role>>> bulkLoader()
    {
        return () ->
        {
            ...
            UntypedResultSet results = process("SELECT * FROM system_auth.roles", apippiAuthorizer.authReadConsistencyLevel());
