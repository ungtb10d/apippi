...
import org.apache.apippi.audit.AuditLogManagerMBean;
import org.apache.apippi.audit.AuditLogOptions;
import org.apache.apippi.audit.AuditLogOptionsCompositeData;
import com.google.common.collect.ImmutableMap;
import org.apache.apippi.auth.AuthCache;
import org.apache.apippi.auth.AuthCacheMBean;
import org.apache.apippi.auth.NetworkPermissionsCache;
import org.apache.apippi.auth.NetworkPermissionsCacheMBean;
import org.apache.apippi.auth.PasswordAuthenticator;
import org.apache.apippi.auth.PermissionsCache;
import org.apache.apippi.auth.PermissionsCacheMBean;
import org.apache.apippi.auth.RolesCache;
import org.apache.apippi.auth.RolesCacheMBean;
import org.apache.apippi.auth.jmx.AuthorizationProxy;
import org.apache.apippi.batchlog.BatchlogManager;
import org.apache.apippi.batchlog.BatchlogManagerMBean;
import org.apache.apippi.db.ColumnFamilyStoreMBean;
import org.apache.apippi.db.compaction.CompactionManager;
import org.apache.apippi.db.compaction.CompactionManagerMBean;
...
import org.apache.apippi.gms.GossiperMBean;
import org.apache.apippi.hints.HintsService;
import org.apache.apippi.hints.HintsServiceMBean;
...
import org.apache.apippi.metrics.TableMetrics;
import org.apache.apippi.metrics.ThreadPoolMetrics;
import org.apache.apippi.net.MessagingService;
import org.apache.apippi.net.MessagingServiceMBean;
import org.apache.apippi.service.ActiveRepairServiceMBean;
...
import org.apache.apippi.service.GCInspectorMXBean;
import org.apache.apippi.service.StorageProxy;
import org.apache.apippi.service.StorageProxyMBean;
import org.apache.apippi.service.StorageServiceMBean;
import org.apache.apippi.streaming.StreamManagerMBean;
import org.apache.apippi.streaming.StreamState;
import org.apache.apippi.streaming.management.StreamStateCompositeData;
...
import org.apache.apippi.tools.nodetool.GetTimeout;
import org.apache.apippi.utils.NativeLibrary;

/**
 * JMX client operations for apippi.
 */
public class NodeProbe implements AutoCloseable
{
    private static final String fmtUrl = "service:jmx:rmi:///jndi/rmi://%s:%d/jmxrmi";
    private static final String ssObjName = "org.apache.apippi.db:type=StorageService";
    private static final int defaultPort = 7199;

    static long JMX_NOTIFICATION_POLL_INTERVAL_SECONDS = Long.getLong("apippi.nodetool.jmx_notification_poll_interval_seconds", TimeUnit.SECONDS.convert(5, TimeUnit.MINUTES));
    ...
    protected void connect() throws IOException
    {
        ...
        if (host.contains(":"))
        {
            // Use square brackets to surround IPv6 addresses to fix apippi-7669 and apippi-17581
            host = "[" + host + "]";
        }
    ...
    public void close() throws IOException
    {
        ...
        catch (ConnectException e)
        {
            ...
            System.out.println("apippi has shutdown.");
        }
    }
    ...
    {
        String cachePath = "org.apache.apippi.db:type=Caches";
    ...
    public EndpointSnitchInfoMBean getEndpointSnitchInfoProxy()
    {
        try
        {
            return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.apippi.db:type=EndpointSnitchInfo"), EndpointSnitchInfoMBean.class);
        }
        catch (MalformedObjectNameException e)
        {
            throw new RuntimeException(e);
    ...
    public DynamicEndpointSnitchMBean getDynamicEndpointSnitchInfoProxy()
    {
        try
        {
            return JMX.newMBeanProxy(mbeanServerConn, new ObjectName("org.apache.apippi.db:type=DynamicEndpointSnitch"), DynamicEndpointSnitchMBean.class);
        }
        catch (MalformedObjectNameException e)
        {
            throw new RuntimeException(e);
    ...
    public ColumnFamilyStoreMBean getCfsProxy(String ks, String cf)
    {
        ...
        try
        {
            ...
            Set<ObjectName> beans = mbeanServerConn.queryNames(
                    new ObjectName("org.apache.apippi.db:type=*" + type +",keyspace=" + ks + ",columnfamily=" + cf), null);
            ...
            switch(metricName)
            {
                ...
                case "Size":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Cache,scope=" + cacheType + ",name=" + metricName),
                            apippiMetricsRegistry.JmxGaugeMBean.class).getValue();
                case "Requests":
                case "Hits":
                case "Misses":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Cache,scope=" + cacheType + ",name=" + metricName),
                            apippiMetricsRegistry.JmxMeterMBean.class).getCount();
                case "MissLatency":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Cache,scope=" + cacheType + ",name=" + metricName),
                            apippiMetricsRegistry.JmxTimerMBean.class).getMean();
                case "MissLatencyUnit":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Cache,scope=" + cacheType + ",name=MissLatency"),
                            apippiMetricsRegistry.JmxTimerMBean.class).getDurationUnit();
                default:
    ...
    private static Multimap<String, String> getJmxThreadPools(MBeanServerConnection mbeanServerConn)
    {
        try
        {
            ...
            Set<ObjectName> threadPoolObjectNames = mbeanServerConn.queryNames(
                    new ObjectName("org.apache.apippi.metrics:type=ThreadPools,*"),
                    null);
    ...
    {
      String name = String.format("org.apache.apippi.metrics:type=ThreadPools,path=%s,scope=%s,name=%s",
    ...
    public Object getColumnFamilyMetric(String ks, String cf, String metricName)
    {
        try
        {
            ...
            if (!Strings.isNullOrEmpty(ks) && !Strings.isNullOrEmpty(cf))
            {
                ...
                oName = new ObjectName(String.format("org.apache.apippi.metrics:type=%s,keyspace=%s,scope=%s,name=%s", type, ks, cf, metricName));
            }
            else if (!Strings.isNullOrEmpty(ks))
            {
                oName = new ObjectName(String.format("org.apache.apippi.metrics:type=Keyspace,keyspace=%s,name=%s", ks, metricName));
            }
            else
            {
                oName = new ObjectName(String.format("org.apache.apippi.metrics:type=Table,name=%s", metricName));
            }
            switch(metricName)
            {
                ...
                case "SnapshotsSize":
                    return JMX.newMBeanProxy(mbeanServerConn, oName, apippiMetricsRegistry.JmxGaugeMBean.class).getValue();
                ...
                case "DroppedMutations":
                    return JMX.newMBeanProxy(mbeanServerConn, oName, apippiMetricsRegistry.JmxCounterMBean.class).getCount();
                case "CoordinatorReadLatency":
                case "CoordinatorScanLatency":
                case "ReadLatency":
                case "WriteLatency":
                    return JMX.newMBeanProxy(mbeanServerConn, oName, apippiMetricsRegistry.JmxTimerMBean.class);
                case "LiveScannedHistogram":
                case "SSTablesPerReadHistogram":
                case "TombstoneScannedHistogram":
                    return JMX.newMBeanProxy(mbeanServerConn, oName, apippiMetricsRegistry.JmxHistogramMBean.class);
                default:
    ...
    public apippiMetricsRegistry.JmxTimerMBean getProxyMetric(String scope)
    {
        ...
        {
            return JMX.newMBeanProxy(mbeanServerConn,
                    new ObjectName("org.apache.apippi.metrics:type=ClientRequest,scope=" + scope + ",name=Latency"),
                    apippiMetricsRegistry.JmxTimerMBean.class);
        }
        catch (MalformedObjectNameException e)
        {
            throw new RuntimeException(e);
    ...
    public apippiMetricsRegistry.JmxTimerMBean getMessagingQueueWaitMetrics(String verb)
    {
        ...
        {
            return JMX.newMBeanProxy(mbeanServerConn,
                                     new ObjectName("org.apache.apippi.metrics:name=" + verb + "-WaitLatency,type=Messaging"),
                                     apippiMetricsRegistry.JmxTimerMBean.class);
        }
        catch (MalformedObjectNameException e)
        {
    ...
    public Object getCompactionMetric(String metricName)
    {
        try
        {
            switch(metricName)
            {
                case "BytesCompacted":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Compaction,name=" + metricName),
                            apippiMetricsRegistry.JmxCounterMBean.class);
                case "CompletedTasks":
                case "PendingTasks":
                ...
                case "TotalCompactionsCompleted":
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Compaction,name=" + metricName),
                            apippiMetricsRegistry.JmxMeterMBean.class);
                default:
        ...
        try
        {
            switch(metricName)
            {
                ...
                case "clientsByProtocolVersion": // number of native clients by username
                    return JMX.newMBeanProxy(mbeanServerConn,
                            new ObjectName("org.apache.apippi.metrics:type=Client,name=" + metricName),
                            apippiMetricsRegistry.JmxGaugeMBean.class).getValue();
                default:
    ...
    public long getStorageMetric(String metricName)
    {
        ...
        {
            return JMX.newMBeanProxy(mbeanServerConn,
                    new ObjectName("org.apache.apippi.metrics:type=Storage,name=" + metricName),
                    apippiMetricsRegistry.JmxCounterMBean.class).getCount();
        }
        catch (MalformedObjectNameException e)
        {
            throw new RuntimeException(e);
    ...
    public Double[] metricPercentilesAsArray(apippiMetricsRegistry.JmxHistogramMBean metric)
    {
    ...
    public Double[] metricPercentilesAsArray(apippiMetricsRegistry.JmxTimerMBean metric)
    {
...
class ColumnFamilyStoreMBeanIterator implements Iterator<Map.Entry<String, ColumnFamilyStoreMBean>>
{
    ...
    {
        ObjectName query = new ObjectName("org.apache.apippi.db:type=" + type +",*");
