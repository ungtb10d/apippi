...
Installation:
============
change directory to <apippi_src_dir>/examples/triggers
run "ant jar"
Copy build/trigger-example.jar to <apippi_conf>/triggers/
Copy conf/* to <apippi_home>/conf/
...
Configure the trigger on the table:
    CREATE TRIGGER test1 ON test.test
        USING 'org.apache.apippi.triggers.AuditTrigger';
