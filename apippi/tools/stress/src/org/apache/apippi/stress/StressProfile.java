 ...
 *
 */
package org.apache.apippi.stress;
...
import org.apache.apippi.cql3.statements.ModificationStatement;
...
import org.apache.apippi.stress.generate.*;
import org.apache.apippi.stress.generate.values.*;
import org.apache.apippi.stress.operations.userdefined.CASQuery;
import org.apache.apippi.stress.operations.userdefined.SchemaInsert;
import org.apache.apippi.stress.operations.userdefined.SchemaQuery;
import org.apache.apippi.stress.operations.userdefined.SchemaStatement;
import org.apache.apippi.stress.operations.userdefined.TokenRangeQuery;
import org.apache.apippi.stress.operations.userdefined.ValidatingSchemaQuery;
import org.apache.apippi.stress.report.Timer;
import org.apache.apippi.stress.settings.*;
import org.apache.apippi.stress.util.JavaDriverClient;
import org.apache.apippi.stress.util.ResultLogger;
...
public class StressProfile implements Serializable
{
    ...
    public void maybeCreateSchema(StressSettings settings)
    {
        if (!schemaCreated)
        {
            ...
            if (keyspaceCql != null)
            {
                try
                {
                    client.execute(keyspaceCql, org.apache.apippi.db.ConsistencyLevel.ONE);
                }
            ...
            client.execute("use " + keyspaceName, org.apache.apippi.db.ConsistencyLevel.ONE);

            if (tableCql != null)
            {
                try
                {
                    client.execute(tableCql, org.apache.apippi.db.ConsistencyLevel.ONE);
                }
                catch (AlreadyExistsException e)
                {
                }
            ...
            if (extraSchemaDefinitions != null)
            {
                for (String extraCql : extraSchemaDefinitions)
                {
                    ...
                    try
                    {
                        client.execute(extraCql, org.apache.apippi.db.ConsistencyLevel.ONE);
                    }
                    catch (AlreadyExistsException e)
                    {
                    }
    ...
    public void truncateTable(StressSettings settings)
    {
        ...
        client.execute(cql, org.apache.apippi.db.ConsistencyLevel.ONE);
    ...
    {
        org.apache.apippi.schema.TableMetadata metadata = CreateTableStatement.parse(tableCql, keyspaceName).build();
