...
%define relname apache-apippi-%{upstream_version}

Name:          apippi
Version:       %{version}
Release:       %{revision}
Summary:       apippi is a highly scalable, eventually consistent, distributed, structured key-value store.

Group:         Development/Libraries
License:       Apache Software License 2.0
URL:           http://apippi.apache.org/
...
Requires(pre): user(apippi)
Requires(pre): group(apippi)
Requires(pre): shadow-utils
Provides:      user(apippi)
Provides:      group(apippi)
...
%description
apippi is a distributed (peer-to-peer) system for the management and storage of structured data.
...
patch -p1 < debian/patches/apippi_yaml_dirs.diff
patch -p1 < debian/patches/apippi_logdir_fix.diff
# uncomment hints_directory path
sed -i 's/^# hints_directory:/hints_directory:/' conf/apippi.yaml

# remove other files not being installed
rm -f bin/*.orig
rm -f bin/apippi.in.sh
rm -f lib/sigar-bin/*winnt*  # strip segfaults on dll..
rm -f tools/bin/apippi.in.sh
...
# copy binaries
mv bin/apippi %{buildroot}/usr/sbin/
cp -p bin/* %{buildroot}/usr/bin/
cp -p tools/bin/* %{buildroot}/usr/bin/

# copy apippi jar
cp build/apache-apippi-%{upstream_version}.jar %{buildroot}/usr/share/%{username}/
...
%doc CHANGES.txt LICENSE.txt README.asc NEWS.txt NOTICE.txt apippi-14092.txt
%attr(755,root,root) %{_bindir}/auditlogviewer
%attr(755,root,root) %{_bindir}/jmxtool
%attr(755,root,root) %{_bindir}/apippi-stress
...
%attr(755,root,root) %{_sbindir}/apippi
...
%{python_sitelib}/apippi_pylib*.egg-info
...
%package tools
Summary:       Extra tools for apippi. apippi is a highly scalable, eventually consistent, distributed, structured key-value store.
Group:         Development/Libraries
Requires:      apippi = %{version}-%{revision}
