 ...
 */
package org.apache.apippi.cql3.validation.operations;
...
import org.apache.apippi.db.memtable.SkipListMemtable;
import org.apache.apippi.db.memtable.TestMemtable;
...
import org.apache.apippi.schema.MemtableParams;
import org.apache.apippi.schema.SchemaConstants;
import org.apache.apippi.schema.SchemaKeyspaceTables;
...
public class AlterTest extends CQLTester
{
    ...
    public void testCreateAlterKeyspaces() throws Throwable
    {
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                   row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                   row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                   row(ks1, false, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "1")),
                   row(ks2, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")));
        ...
        assertRows(execute("SELECT table_name, compaction FROM system_schema.tables WHERE keyspace_name='" + ks1 + "'"),
                   row("cf1", map("class", "org.apache.apippi.db.compaction.SizeTieredCompactionStrategy",
                                  "min_threshold", "7",
                                  "max_threshold", "32")));
    }
    ...
    public void testCreateAlterNetworkTopologyWithDefaults() throws Throwable
    {
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(ks1, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "1", DATA_CENTER_REMOTE, "3")));
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(ks1, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER_REMOTE, "3")));
        ...
        assertRows(execute("SELECT table_name, compaction FROM system_schema.tables WHERE keyspace_name='" + ks1 + "'"),
                   row("tbl1", map("class", "org.apache.apippi.db.compaction.SizeTieredCompactionStrategy",
                                  "min_threshold", "7",
                                  "max_threshold", "32")));
        metadata.clearUnsafe();
    }
    ...
    public void testCreateSimpleAlterNTSDefaults() throws Throwable
    {
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(ks1, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "2")));
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(ks1, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "2", DATA_CENTER_REMOTE, "2")));
    ...
    public void testDefaultRF() throws Throwable
    {
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks1, true, map("class","org.apache.apippi.locator.SimpleStrategy","replication_factor", Integer.toString(DatabaseDescriptor.getDefaultKeyspaceRF()))),
                                        row(ks2, true, map("class","org.apache.apippi.locator.SimpleStrategy","replication_factor", "2")),
                                        row(ks3, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER,
                                        ...
                                        row(ks4, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "2", DATA_CENTER_REMOTE, "2")));
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks2, true, map("class","org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "2", DATA_CENTER_REMOTE, "2")));
        // with specifying RF
        schemaChange("ALTER KEYSPACE " + ks1 + " WITH replication={ 'class' : 'NetworkTopologyStrategy', 'replication_factor': '1' } AND durable_writes=true");
        // verify that explicitly mentioned RF is taken into account
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks1, true, map("class","org.apache.apippi.locator.NetworkTopologyStrategy", DATA_CENTER, "1", DATA_CENTER_REMOTE, "1")));
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks4, true, map("class","org.apache.apippi.locator.SimpleStrategy","replication_factor", Integer.toString(DatabaseDescriptor.getDefaultKeyspaceRF()))));
        // with specifying RF
        schemaChange("ALTER KEYSPACE " + ks3 + " WITH replication={ 'class' : 'SimpleStrategy', 'replication_factor' : '1' } AND durable_writes=true");
        // verify that explicitly mentioned RF is taken into account
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks3, true, map("class","org.apache.apippi.locator.SimpleStrategy","replication_factor", "1")));
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks5, true, map("class","org.apache.apippi.locator.SimpleStrategy","replication_factor", oldRF)));
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(ks6, true, map("class", "org.apache.apippi.locator.NetworkTopologyStrategy",
    ...
    public void testAlterTableWithMemtable() throws Throwable
    {
        ...
        assertAlterTableThrowsException(ConfigurationException.class,
                                        "Memtable class org.apache.apippi.db.memtable.SkipListMemtable does not accept any futher parameters, but {invalid=throw} were given.",
                                        "ALTER TABLE %s"
        ...
        assertAlterTableThrowsException(ConfigurationException.class,
                                        "Memtable class org.apache.apippi.db.memtable.TestMemtable does not accept any futher parameters, but {invalid=throw} were given.",
                                        "ALTER TABLE %s"
    ...
    {
        createTable("CREATE TABLE %s (a text, b int, c int, primary key (a, b))");
        assertSchemaOption("compression", map("chunk_length_in_kb", "16", "class", "org.apache.apippi.io.compress.LZ4Compressor"));

        alterTable("ALTER TABLE %s WITH compression = { 'class' : 'SnappyCompressor', 'chunk_length_in_kb' : 32 };");
        assertSchemaOption("compression", map("chunk_length_in_kb", "32", "class", "org.apache.apippi.io.compress.SnappyCompressor"));

        alterTable("ALTER TABLE %s WITH compression = { 'class' : 'LZ4Compressor', 'chunk_length_in_kb' : 64 };");
        assertSchemaOption("compression", map("chunk_length_in_kb", "64", "class", "org.apache.apippi.io.compress.LZ4Compressor"));

        alterTable("ALTER TABLE %s WITH compression = { 'class' : 'LZ4Compressor', 'min_compress_ratio' : 2 };");
        assertSchemaOption("compression", map("chunk_length_in_kb", "16", "class", "org.apache.apippi.io.compress.LZ4Compressor", "min_compress_ratio", "2.0"));

        alterTable("ALTER TABLE %s WITH compression = { 'class' : 'LZ4Compressor', 'min_compress_ratio' : 1 };");
        assertSchemaOption("compression", map("chunk_length_in_kb", "16", "class", "org.apache.apippi.io.compress.LZ4Compressor", "min_compress_ratio", "1.0"));
    ...
    /**
     * Test for apippi-13337. Checks that dropping a column when a sstable contains only data for that column
     * works properly.
     */
    @Test
    ...
    public void testAlterDropCompactStorageDisabled() throws Throwable
    {
        ...
        assertInvalidMessage("DROP COMPACT STORAGE is disabled. Enable in apippi.yaml to use.", "ALTER TABLE %s DROP COMPACT STORAGE");
    }

    /**
     * Test for apippi-14564
     */
    @Test
    ...
    public void testAlterKeyspaceWithIfExists() throws Throwable
    {
        ...
        assertRowsIgnoringOrderAndExtra(execute("SELECT keyspace_name, durable_writes, replication FROM system_schema.keyspaces"),
                                        row(KEYSPACE, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(KEYSPACE_PER_TEST, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")),
                                        row(ks1, true, map("class", "org.apache.apippi.locator.SimpleStrategy", "replication_factor", "1")));
