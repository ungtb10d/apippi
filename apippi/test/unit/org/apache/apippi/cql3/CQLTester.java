...
import org.apache.apippi.metrics.ClientMetrics;
import org.apache.apippi.schema.*;
...
import org.apache.apippi.transport.*;
...
public abstract class CQLTester
{
     ...
     * The super user
     */
    private static final User SUPER_USER = new User("apippi", "apippi");

    protected static final Logger logger = LoggerFactory.getLogger(CQLTester.class);

    public static final String KEYSPACE = "cql_test_keyspace";
    public static final String KEYSPACE_PER_TEST = "cql_test_keyspace_alt";
    protected static final boolean USE_PREPARED_VALUES = Boolean.valueOf(System.getProperty("apippi.test.use_prepared", "true"));
    protected static final boolean REUSE_PREPARED = Boolean.valueOf(System.getProperty("apippi.test.reuse_prepared", "true"));
    protected static final long ROW_CACHE_SIZE_IN_MIB = new DataStorageSpec.LongMebibytesBound(System.getProperty("apippi.test.row_cache_size", "0MiB")).toMebibytes();
    ...
    private static org.apache.apippi.transport.Server server;
    ...
    {
        int port = jmxPort == 0 ? Integer.getInteger("apippi.jmx.local.port", 7199) : jmxPort;
    ...
    public static List<String> buildapippiStressArgs(List<String> args)
    {
        List<String> allArgs = new ArrayList<>();
        allArgs.add("tools/bin/apippi-stress");
        allArgs.addAll(args);
    ...
    protected static void requireAuthentication()
    {
        ...
        DatabaseDescriptor.setNetworkAuthorizer(new AuthTestUtils.LocalapippiNetworkAuthorizer());

        // The apippiRoleManager constructor set the supported and alterable options based on
    ...
    private static Cluster initClientCluster(User user, ProtocolVersion version)
    {
        SocketOptions socketOptions =
                new SocketOptions().setConnectTimeoutMillis(Integer.getInteger("apippi.test.driver.connection_timeout_ms",
                                                                               DEFAULT_CONNECT_TIMEOUT_MILLIS)) // default is 5000
                                   .setReadTimeoutMillis(Integer.getInteger("apippi.test.driver.read_timeout_ms",
    ...
    {
        String metricName = "org.apache.apippi.metrics.Client.PausedConnections";
        Map<String, Gauge> metrics = apippiMetricsRegistry.Metrics.getGauges((name, metric) -> name.equals(metricName));
        if (metrics.size() != 1)
