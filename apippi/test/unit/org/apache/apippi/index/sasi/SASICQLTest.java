...
public class SASICQLTest extends CQLTester
{
    ...
    public void testPaging() throws Throwable
    {
        for (boolean forceFlush : new boolean[]{ false, true })
        {
            ...
            createIndex("CREATE CUSTOM INDEX ON %s (v) USING 'org.apache.apippi.index.sasi.SASIIndex';");
        ...
        for (String mode : new String[]{ "PREFIX", "CONTAINS", "SPARSE"})
        {
            for (boolean forceFlush : new boolean[]{ false, true })
            {
                try
                {
                    ...
                    createIndex(String.format("CREATE CUSTOM INDEX ON %%s (v) USING 'org.apache.apippi.index.sasi.SASIIndex' WITH OPTIONS = {'is_literal': 'false', 'mode': '%s'};", mode));
    ...
    public void testStringCompare() throws Throwable
    {
        for (String mode : new String[]{ "PREFIX", "CONTAINS"})
        {
            for (boolean forceFlush : new boolean[]{ false, true })
            {
                ...
                {
                    createTable("CREATE TABLE %s (pk int primary key, v text);");
                    createIndex(String.format("CREATE CUSTOM INDEX ON %%s (v) USING 'org.apache.apippi.index.sasi.SASIIndex' WITH OPTIONS = {'mode': '%s'};", mode));
