...
import static org.apache.apippi.security.PEMBasedSslContextFactory.ConfigKey.ENCODED_CERTIFICATES;
import static org.apache.apippi.security.PEMBasedSslContextFactory.ConfigKey.ENCODED_KEY;
import static org.apache.apippi.security.PEMBasedSslContextFactory.ConfigKey.KEY_PASSWORD;
...
public class PEMBasedSslContextFactoryTest
{
    ...
    private void addKeyStoreOptions(Map<String, Object> config)
    {
        ...
        config.put(KEY_PASSWORD.getKeyName(), "apippi");
    }
    ...
    {
        config.put("truststore", "test/conf/apippi_ssl_test.truststore.pem");
    }

    private void addFileBaseKeyStoreOptions(Map<String, Object> config)
    {
        config.put("keystore", "test/conf/apippi_ssl_test.keystore.pem");
        config.put("keystore_password", "apippi");
    }

    private void addFileBaseUnencryptedKeyStoreOptions(Map<String, Object> config)
    {
        config.put("keystore", "test/conf/apippi_ssl_test.unencrypted_keystore.pem");
    }
    ...
    public void getSslContextOpenSSL() throws IOException
    {
        ...
        , new HashMap<>());
        EncryptionOptions options = new EncryptionOptions().withTrustStore("test/conf/apippi_ssl_test.truststore.pem")
                                                           .withKeyStore("test/conf/apippi_ssl_test.keystore.pem")
                                                           .withKeyStorePassword("apippi")
    ...
    public void getSslContextOpenSSLOutboundKeystore() throws IOException
    {
        ParameterizedClass sslContextFactory = new ParameterizedClass(PEMBasedSslContextFactory.class.getSimpleName()
        , new HashMap<>());
        EncryptionOptions.ServerEncryptionOptions options = new EncryptionOptions.ServerEncryptionOptions().withTrustStore("test/conf/apippi_ssl_test.truststore.pem")
                                                                                                           .withKeyStore("test/conf/apippi_ssl_test.keystore.pem")
                                                                                                           .withKeyStorePassword("apippi")
                                                                                                           .withOutboundKeystore("test/conf/apippi_ssl_test.keystore.pem")
                                                                                                           .withOutboundKeystorePassword("apippi")
                                                                                                           .withRequireClientAuth(false)
                                                                                                           .withCipherSuites("TLS_RSA_WITH_AES_128_CBC_SHA")
                                                                                                           .withSslContextFactory(sslContextFactory);
    ...
    public void testDisableOpenSslForInJvmDtests()
    {
        ...
        System.setProperty("apippi.disable_tcactive_openssl", "true");
        Map<String, Object> config = new HashMap<>();
        config.putAll(commonConfig);

        PEMBasedSslContextFactory sslContextFactory = new PEMBasedSslContextFactory(config);
        Assert.assertEquals(SslProvider.JDK, sslContextFactory.getSslProvider());
        System.clearProperty("apippi.disable_tcactive_openssl");
    }
