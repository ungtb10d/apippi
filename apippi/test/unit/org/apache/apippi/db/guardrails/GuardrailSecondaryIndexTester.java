...
public class GuardrailSecondaryIndexTester extends GuardrailTester
{
    ...
    public void testCustomIndex() throws Throwable
    {
        ...
        setGuardrail(false);
        assertFails(format("CREATE CUSTOM INDEX ON %%s (%s) USING 'org.apache.apippi.index.sasi.SASIIndex'", "v4"),
                    format("Creating secondary indexes", currentTable())
        );

        // Confirm custom creation will work on flip
        setGuardrail(true);
        assertValid(format("CREATE CUSTOM INDEX ON %%s (%s) USING 'org.apache.apippi.index.sasi.SASIIndex'", "v4"));
    }
}
