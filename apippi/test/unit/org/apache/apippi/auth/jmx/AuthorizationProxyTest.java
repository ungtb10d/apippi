...
package org.apache.apippi.auth.jmx;
...
public class AuthorizationProxyTest
{
    ...
    JMXResource hintsBean = JMXResource.mbean("org.apache.apippi.hints:type=HintsService");
    JMXResource batchlogBean = JMXResource.mbean("org.apache.apippi.db:type=BatchlogManager");
    JMXResource customBean = JMXResource.mbean("org.apache.apippi:type=CustomBean,property=foo");
    ...
    {
        JMXResource customWildcard = JMXResource.mbean("org.apache.apippi:*");
        Map<RoleResource, Set<PermissionDetails>> permissions =
        ...
        // the grant on org.apache.apippi:* shouldn't permit us to invoke queryNames with java.lang:*
        assertFalse(proxy.authorize(subject(role1.getRoleName()),
                                    "queryNames",
                                    new Object[]{ objectName(javaLangWildcard), null }));
    }
    ...
    {
        // in this test, permissions are granted on org.apache.apippi:type=CustomBean,property=*
        // and all beans in the org.apache.apippi.hints domain, but
        // but the target of the invocation is org.apache.apippi*:*
        // i.e. the subject has permissions on all CustomBeans and on the HintsService bean, but is
        // attempting to query all names in the org.apache.apippi* domain. The operation should
        // be rejected as the permissions don't cover all known beans matching that domain, due to
        // the BatchLogManager bean.

        JMXResource allCustomBeans = JMXResource.mbean("org.apache.apippi:type=CustomBean,property=*");
        JMXResource allHintsBeans = JMXResource.mbean("org.apache.apippi.hints:*");
        ObjectName allapippiBeans = ObjectName.getInstance("org.apache.apippi*:*");
        ...
        assertFalse(proxy.authorize(subject(role1.getRoleName()),
                                    "queryNames",
                                    new Object[]{ allapippiBeans, null }));
    }
    ...
    private static Subject subject(String roleName)
    {
        ...
        subject.getPrincipals().add(new apippiPrincipal(roleName));
        return subject;
    }
