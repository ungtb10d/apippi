...
package org.apache.apippi.distributed.test.thresholds;
...
import org.apache.apippi.concurrent.SEPExecutor;
...
import org.apache.apippi.distributed.test.JavaDriverUtils;
...
import org.apache.apippi.exceptions.TombstoneAbortException;
...
public class TombstoneCountWarningTest extends TestBaseImpl
{
    ...
    private void failThreshold(String cql, boolean isScan) throws UnknownHostException
    {
        ...
        List<String> warnings = CLUSTER.get(1).callsOnInstance(() -> {
            ...
            try
            {
                QueryProcessor.execute(cql, org.apache.apippi.db.ConsistencyLevel.ALL, QueryState.forInternalCalls());
    ...
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.ClientTombstoneWarnings." + KEYSPACE)).sum();
    }

    private static long totalAborts()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.ClientTombstoneAborts." + KEYSPACE)).sum();
    }

    private static long totalReadAborts()
    {
        return CLUSTER.stream().mapToLong(i ->
                                          i.metrics().getCounter("org.apache.apippi.metrics.ClientRequest.Aborts.Read-ALL")
                                          + i.metrics().getCounter("org.apache.apippi.metrics.ClientRequest.Aborts.RangeSlice")
        ).sum();
    }
