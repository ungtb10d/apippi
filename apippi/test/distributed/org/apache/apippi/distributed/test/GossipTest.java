...
import org.apache.apippi.distributed.api.*;
...
import static org.apache.apippi.distributed.api.TokenSupplier.evenlyDistributedTokens;
import static org.apache.apippi.distributed.impl.DistributedTestSnitch.toapippiInetAddressAndPort;
import static org.apache.apippi.distributed.shared.ClusterUtils.runAndWaitForLogs;
import static org.apache.apippi.distributed.shared.NetworkTopology.singleDcNetworkTopology;
...
public class GossipTest extends TestBaseImpl
{
                                      ...
                                      .createWithoutStarting())
        {
            ...
            for (int i = 1 ; i <= liveCount ; ++i)
            {
                cluster.get(i).acceptsOnInstance((InetSocketAddress address) -> {
                    EndpointState ep;
                    InetAddressAndPort endpoint = toapippiInetAddressAndPort(address);
                ...
                cluster.get(late)
                       .appliesOnInstance((InetSocketAddress address) ->
                                          StorageService.instance.getTokenMetadata()
                                                                 .getTokens(toapippiInetAddressAndPort(address))
                                                                 .stream()
                                                                 .map(Object::toString)
                                                                 .collect(Collectors.toList()))
                       .apply(failAddress);
    ...
    public void gossipShutdownUpdatesTokenMetadata() throws Exception
    {
                                      ...
                                      .start())
        {
            ...
            // apippi-16796 this doesn't update TokenMetadata, so they maintain pending ranges for the down node
    ...
    public void restartGossipOnGossippingOnlyMember() throws Throwable
    {
                                        ...
                                        .start())
        {
            ...
            withProperty("apippi.join_ring", Boolean.toString(false), () -> gossippingOnlyMember.startup(cluster));
    ...
    void assertPendingRangesForPeer(final boolean expectPending, final InetSocketAddress movingAddress, final Cluster cluster)
    {
        for (IInvokableInstance inst : new IInvokableInstance[]{ cluster.get(1), cluster.get(3)})
        {
            boolean hasPending = inst.appliesOnInstance((InetSocketAddress address) -> {
                InetAddressAndPort peer = toapippiInetAddressAndPort(address);
