...
import static org.apache.apippi.distributed.shared.ClusterUtils.getDirectories;
import static org.apache.apippi.distributed.shared.ClusterUtils.stopUnchecked;
import static org.apache.apippi.distributed.shared.ClusterUtils.updateAddress;
...
public class IPMembershipTest extends TestBaseImpl
{
    ...
    public void sameIPFailWithoutReplace() throws IOException
    {
                                      ...
                                      .start())
        {
            ...
            ToolRunner.invokeapippiStress("write", "n=10000", "-schema", "replication(factor=3)", "-port", "native=9042").assertOnExitCode();
            ...
            for (boolean auto_bootstrap : Arrays.asList(true, false))
            {
                ...
                Assertions.assertThatThrownBy(() -> nodeToReplace.startup())
                          .hasMessage("A node with address /127.0.0.3:7012 already exists, cancelling join. Use apippi.replace_address if you want to replace this node.");
            }
        }
    }
    ...
    public void startupNewIP() throws IOException, InterruptedException
    {
        try (Cluster cluster = Cluster.build(3)
                                      .withConfig(c -> c.with(Feature.GOSSIP, Feature.NATIVE_PROTOCOL)
                                                        ...
                                                        .set("endpoint_snitch", "org.apache.apippi.locator.SimpleSnitch"))
                                      .start())
        {
            IInvokableInstance nodeToReplace = cluster.get(3);
            ...
            ToolRunner.invokeapippiStress("read", "n=10000", "no-warmup", "-port", "native=9042").assertOnExitCode();
        }
    }
}
