...
public class LocalReadSizeWarningTest extends AbstractClientSizeWarning
{
    ...
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.LocalReadSize." + KEYSPACE)).toArray();
    }

    @Override
    protected long totalWarnings()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.LocalReadSizeWarnings." + KEYSPACE)).sum();
    }

    @Override
    protected long totalAborts()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.LocalReadSizeAborts." + KEYSPACE)).sum();
    }
}
