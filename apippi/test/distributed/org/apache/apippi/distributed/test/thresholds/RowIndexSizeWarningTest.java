...
public class RowIndexSizeWarningTest extends AbstractClientSizeWarning
{
    ...
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.RowIndexSize." + KEYSPACE)).toArray();
    }

    @Override
    protected long totalWarnings()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.RowIndexSizeWarnings." + KEYSPACE)).sum();
    }

    @Override
    protected long totalAborts()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.RowIndexSizeAborts." + KEYSPACE)).sum();
    }
}
