...
import static org.apache.apippi.distributed.shared.AssertUtils.assertRows;
import static org.apache.apippi.distributed.shared.AssertUtils.row;
import static org.apache.apippi.net.Verb.PAXOS2_PREPARE_REQ;
import static org.apache.apippi.net.Verb.PAXOS2_PROPOSE_REQ;
...
public abstract class CASCommonTestCases extends CASTestBase
{
    ...
    public void simpleUpdate() throws Throwable
    {
        ...
        getCluster().coordinator(1).execute("INSERT INTO " + fullTableName + " (pk, ck, v) VALUES (1, 1, 1) IF NOT EXISTS", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM);
        assertRows(getCluster().coordinator(1).execute("SELECT * FROM " + fullTableName + " WHERE pk = 1", org.apache.apippi.distributed.api.ConsistencyLevel.SERIAL),
                   row(1, 1, 1));
        getCluster().coordinator(1).execute("UPDATE " + fullTableName + " SET v = 3 WHERE pk = 1 and ck = 1 IF v = 2", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM);
        assertRows(getCluster().coordinator(1).execute("SELECT * FROM " + fullTableName + " WHERE pk = 1", org.apache.apippi.distributed.api.ConsistencyLevel.SERIAL),
                   row(1, 1, 1));
        getCluster().coordinator(1).execute("UPDATE " + fullTableName + " SET v = 2 WHERE pk = 1 and ck = 1 IF v = 1", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM);
        assertRows(getCluster().coordinator(1).execute("SELECT * FROM " + fullTableName + " WHERE pk = 1", org.apache.apippi.distributed.api.ConsistencyLevel.SERIAL),
                   row(1, 1, 2));
    }
    ...
    public void testRepairIncompletePropose() throws Throwable
    {
        ...
        for (int repairWithout = 1 ; repairWithout <= 3 ; ++repairWithout)
        {
            ...
            {
                getCluster().coordinator(1).execute("INSERT INTO " + fullTableName + " (pk, ck, v) VALUES (?, 1, 1) IF NOT EXISTS", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM, repairWithout);
                Assert.assertTrue(false);
            }
            catch (RuntimeException t)
            {
            ...
            {
                Object[][] rows = getCluster().coordinator(1).execute("SELECT * FROM " + fullTableName + " WHERE pk = ?", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM, repairWithout);
    ...
    public void testRepairIncompleteCommit() throws Throwable
    {
        ...
        for (int repairWithout = 1 ; repairWithout <= 3 ; ++repairWithout)
        {
            ...
            try (AutoCloseable drop = drop(getCluster(), repairWith, to(repairWithout), to(), to()))
            {
                ...
                assertRows(getCluster().coordinator(repairWith).execute("SELECT * FROM " + fullTableName + " WHERE pk = ?", org.apache.apippi.distributed.api.ConsistencyLevel.QUORUM, repairWithout),
