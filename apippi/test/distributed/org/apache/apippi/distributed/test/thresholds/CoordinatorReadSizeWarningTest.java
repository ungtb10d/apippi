...
public class CoordinatorReadSizeWarningTest extends AbstractClientSizeWarning
{
    ...
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.CoordinatorReadSize." + KEYSPACE)).toArray();
    }

    @Override
    protected long totalWarnings()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.CoordinatorReadSizeWarnings." + KEYSPACE)).sum();
    }

    @Override
    protected long totalAborts()
    {
        return CLUSTER.stream().mapToLong(i -> i.metrics().getCounter("org.apache.apippi.metrics.keyspace.CoordinatorReadSizeAborts." + KEYSPACE)).sum();
    }
}
