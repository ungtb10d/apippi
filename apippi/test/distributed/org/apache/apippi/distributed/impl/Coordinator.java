...
import org.apache.apippi.distributed.api.QueryResult;
import org.apache.apippi.distributed.api.QueryResults;
...
public class Coordinator implements ICoordinator
{
    ...
    public static org.apache.apippi.db.ConsistencyLevel toapippiCL(ConsistencyLevel cl)
    {
        try
        {
            return org.apache.apippi.db.ConsistencyLevel.fromCode(cl.code);
        }
    ...
    protected static org.apache.apippi.db.ConsistencyLevel toapippiSerialCL(ConsistencyLevel cl)
    {
        return toapippiCL(cl == null ? ConsistencyLevel.SERIAL : cl);
    }
    ...
    public static SimpleQueryResult unsafeExecuteInternal(String query, ConsistencyLevel serialConsistencyLevel, ConsistencyLevel commitConsistencyLevel, Object[] boundValues)
    {
        ...
        try
        {
            ResultMessage res = prepared.execute(QueryState.forInternalCalls(),
                                   QueryOptions.create(toapippiCL(commitConsistencyLevel),
                                                       boundBBValues,
                                                       false,
                                                       Integer.MAX_VALUE,
                                                       null,
                                                       toapippiSerialCL(serialConsistencyLevel),
    ...
    public QueryResult executeWithPagingWithResult(String query, ConsistencyLevel consistencyLevelOrigin, int pageSize, Object... boundValues)
    {
        ...
        return instance.sync(() -> {
            ...
            QueryOptions initialOptions = QueryOptions.create(toapippiCL(consistencyLevel),
                                                              boundBBValues,
                                                              false,
                                                              pageSize,
                                                              null,
            ...
            Iterator<Object[]> iter = new Iterator<Object[]>() {
                ...
                public boolean hasNext()
                {
                    ...
                    QueryOptions nextOptions = QueryOptions.create(toapippiCL(consistencyLevel),
                                                                   boundBBValues,
                                                                   true,
                                                                   pageSize,
