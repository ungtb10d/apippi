...
import static org.apache.apippi.simulator.asm.MethodLogger.Level.NONE;
import static org.apache.apippi.simulator.asm.MethodLogger.Level.valueOf;

// TODO (config): support logging only for packages/classes matching a pattern
interface MethodLogger
{
    static final Level LOG = valueOf(System.getProperty("apippi.test.simulator.print_asm", "none").toUpperCase());
    static final Set<TransformationKind> KINDS = System.getProperty("apippi.test.simulator.print_asm_opts", "").isEmpty()
                                                 ? EnumSet.allOf(TransformationKind.class)
                                                 : stream(System.getProperty("apippi.test.simulator.print_asm_opts", "").split(","))
                                                   .map(TransformationKind::valueOf)
                                                   .collect(() -> EnumSet.noneOf(TransformationKind.class), Collection::add, Collection::addAll);
    static final Pattern LOG_CLASSES = System.getProperty("apippi.test.simulator.print_asm_classes", "").isEmpty()
                                                 ? null
                                                 : Pattern.compile(System.getProperty("apippi.test.simulator.print_asm_classes", ""));
